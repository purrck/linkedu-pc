...
<template>
  <div class="InterestCourseClassStudentCheck">
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="interestCourseClassName" align="center" label="兴趣班" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.interestCourseClassName" class="one-line">{{scope.row.interestCourseClassName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
                  <el-table-column prop="checkedDt" label="时间" align="center" sortable="custom"></el-table-column>
                  <el-table-column prop="gotClassHour" align="center" label="学时" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.gotClassHour" class="one-line">{{scope.row.gotClassHour}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="isCheckException" label="是否考勤" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isCheckException == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
                  <!--<el-table-column prop="remarkCheckException" align="center" label="缺勤原因" sortable="custom">-->
                    <!--<template slot-scope="scope">-->
                      <!--<div v-if="scope.row.remarkCheckException" class="one-line">{{scope.row.remarkCheckException}}</div>-->
                      <!--<div v-else>无</div>-->
                    <!--</template>-->
                  <!--</el-table-column>-->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.studentCheckId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.studentCheckId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.studentCheckId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/SpecialEducation/detailInterestCourseClassStudentCheck/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/SpecialEducation/InterestCourseClassStudentCheckTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/SpecialEducation/detailInterestCourseClassStudentCheck/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/SpecialEducation/detailInterestCourseClassStudentCheck/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
