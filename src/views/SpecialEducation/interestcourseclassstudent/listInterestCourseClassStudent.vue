<template>
  <div class="InterestCourseClassStudent">
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="兴趣班:" prop="interestCourseClassId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.interestCourseClassId" maxlength="30"></el-input>
          </el-form-item>
          ...
          <!--<el-form-item label="参与学习学时:"  prop="classHourStudyed" class="w140">-->
          <!--<el-input class="input-v2-rao" v-model="searchForm.classHourStudyed" maxlength="30"></el-input>-->
          <!--</el-form-item>-->
          <!--<el-form-item label="上次上课时间:" prop="lastStudyDt" class="w140">-->
          <!--<el-date-picker class="date-v2-rao" v-model="searchForm.lastStudyDt" type="date"-->
          <!--value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>-->
          <!--</el-form-item>-->
          <!--<el-form-item label="考核评价:"  prop="examEvaluation" class="w140">-->
          <!--<el-input class="input-v2-rao" v-model="searchForm.examEvaluation" maxlength="30"></el-input>-->
          <!--</el-form-item>-->
          <el-form-item label="考核分数:" prop="examScore" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.examScore" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="classStudentStatusCode" label="状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.classStudentStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.classStudentStatusCodeStr">{{scope.row.classStudentStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="classHourStudyed" align="center" label="参与学习学时" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.classHourStudyed" class="one-line">{{scope.row.classHourStudyed}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="lastStudyDt" label="上次上课时间" align="center" sortable="custom"></el-table-column>
            <!--<el-table-column prop="examEvaluation" align="center" label="考核评价" sortable="custom">-->
            <!--<template slot-scope="scope">-->
            <!--<div v-if="scope.row.examEvaluation" class="one-line " v-html="scope.row.examEvaluation"></div>-->
            <!--<div v-else>无</div>-->
            <!--</template>-->
            <!--</el-table-column>-->
            <!--<el-table-column prop="examScore" align="center" label="考核分数" sortable="custom">-->
            <!--<template slot-scope="scope">-->
            <!--<div v-if="scope.row.examScore" class="one-line">{{scope.row.examScore}}</div>-->
            <!--<div v-else>无</div>-->
            <!--</template>-->
            <!--</el-table-column>-->
            <!--<el-table-column prop="imagePath" align="center" label="获取证书">-->
            <!--<template slot-scope="scope">-->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.classStudentId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.classStudentId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.classStudentId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="兴趣班学生考勤" trigger="hover">
                      <span slot="reference"
                            @click.stop="toAnother(scope.row.interestCourseClassId ,`listInterestCourseClassStudentCheck`,scope.row.interestCourseClassName,scope.row.studentId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        if (this.$route.query.interestCourseClassId !== undefined) {
          this.hiddenAdd = true
        }
      ...
      toAdd() {
        //safety safetyEdu basedata   this.$route.query.dormitoryRoomId
        let opath = '/SpecialEducation/detailInterestCourseClassStudent/add/' + this.$route.query.interestCourseClassId + '?interestCourseClassName=' + this.$route.query.interestCourseClassName;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/SpecialEducation/InterestCourseClassStudentTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/SpecialEducation/detailInterestCourseClassStudent/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/SpecialEducation/detailInterestCourseClassStudent/detail/' + id;
        this.$router.push({path: opath})
      },
      toAnother(id, path, name, studentId) {
        let opath = '/SpecialEducation/' + path + '?interestCourseClassId=' + id + '&interestCourseClassName=' + name + '&studentId=' + studentId;
        this.$router.push({path: opath})
      },
    }
  }
