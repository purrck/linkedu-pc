    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="心愿类型:" prop="wishTypeId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.wishTypeId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="心愿:" prop="wishName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.wishName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="心愿状态:" prop="wishStatusCode" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.wishStatusCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="wishTypeName" align="center" label="心愿类型" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.wishTypeName" class="one-line">{{scope.row.wishTypeName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="wishName" align="center" label="心愿" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.wishName" class="one-line">{{scope.row.wishName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="wishStatusCode" label="心愿状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.wishStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.wishStatusCodeStr">{{scope.row.wishStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="selectedNum" align="center" label="被选次数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.selectedNum" class="one-line">{{scope.row.selectedNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.wishId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.wishId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.wishId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/specialEdu/detailHabitWish/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/specialEdu/HabitWishTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/specialEdu/detailHabitWish/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/specialEdu/detailHabitWish/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
