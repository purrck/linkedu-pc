    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="花值:" prop="flowerValue" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.flowerValue" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="交易类型：1-收入、2-支出:" prop="exchangeType" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.exchangeType" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="交易时间:" prop="exchangeDt" class="w140">
            <el-date-picker class="date-v2-rao" v-model="searchForm.exchangeDt" type="date"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="flowerValue" align="center" label="花值" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.flowerValue" class="one-line">{{scope.row.flowerValue}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="exchangeType" align="center" label="交易类型：1-收入、2-支出" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.exchangeType" class="one-line">{{scope.row.exchangeType}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="exchangeDt" label="交易时间" align="center" sortable="custom"></el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.flowerHistoryId)"><i
                ...
                <!--<el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">-->
                  <!--<span slot="reference"-->
                        <!--@click.stop="rowDel(scope.row.flowerHistoryId)"><i-->
...
<script>
  import api from "./habitflowerhistoryUrl";
  ...
  export default {
    ...
    methods: {
      ...
      toTotal() {
        let opath = '/specialEdu/HabitFlowerHistoryTotal';
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/specialEdu/detailHabitFlowerHistory/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
