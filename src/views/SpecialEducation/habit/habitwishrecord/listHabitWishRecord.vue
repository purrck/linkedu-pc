    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="心愿名:" prop="wishName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.wishName" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="兑换花值:" prop="flowerValue" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.flowerValue" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="兑换时间:" prop="completedDt" class="w140">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="wishName" align="center" label="心愿名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.wishName" class="one-line">{{scope.row.wishName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="flowerValue" align="center" label="兑换花值" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.flowerValue" class="one-line">{{scope.row.flowerValue}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="completedDt" label="兑换时间" align="center" sortable="custom"></el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.wishRecordId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.wishRecordId)"><i
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toTotal() {
        let opath = '/specialEdu/HabitWishRecordTotal';
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/specialEdu/detailHabitWishRecord/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
