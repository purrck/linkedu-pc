            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.classPlanId)"><i
...
<script>
import api from './courseclassplanUrl'
...
export default {
  ...
  mounted () {
    ...
    commonApi.getCodeIntType('must').then(res => {
      let viewList = res.data.data || []
      for (let mustItem of viewList) {
        this.mustCodeOptions.push({
          'label': mustItem.codeDesc,
          'name': mustItem.codeId
        })
      }
    }, res => {
  ...
  methods: {
    ...
    //删除
    toTotal () {
      let opath = '/safety/CourseClassPlanTotal'
