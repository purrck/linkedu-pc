      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="userName" align="center" label="教师" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="ctdutyCode" label="职责" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.ctdutyCodeStr.split(';')[1]}"
                                  v-if="scope.row.ctdutyCodeStr">{{scope.row.ctdutyCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.courseTeacherId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.courseTeacherId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.courseTeacherId)"><i
...
export default {
  data () {
    return {
      ...
      ctdutyCodeOptions: [{'label': '全部', 'name': ''}],
      ctdutyCodeValue: null,
      form: {
        courseId: null,
      },
      searchForm: {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailCourseTeacher/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/CourseTeacherTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailCourseTeacher/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailCourseTeacher/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
