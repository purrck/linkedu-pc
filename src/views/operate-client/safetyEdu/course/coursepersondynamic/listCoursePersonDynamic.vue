    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="活动:" prop="entityId">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="entityTypeCode" label="活动类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.entityTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.entityTypeCodeStr">{{scope.row.entityTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="readDt" label="浏览时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="readTimes" align="center" label="浏览次数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.readTimes" class="one-line">{{scope.row.readTimes}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="commitDt" label="提交时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="correctDt" label="批改时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="pdstatusCode" label="状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.pdstatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.pdstatusCodeStr">{{scope.row.pdstatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="score" align="center" label="总分数" sortable="custom">
              <template slot-scope="scope">
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.dynamicId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.dynamicId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.dynamicId)"><i
...
<script>
import api from './coursepersondynamicUrl'
...
export default {
  data () {
    return {
      ...
      entityTypeCodeOptions: [{'label': '全部', 'name': ''}],
      entityTypeCodeValue: null,
      pdstatusCodeOptions: [{'label': '全部', 'name': ''}],
      pdstatusCodeValue: null,
      form: {
        entityId: null,
      },
      searchForm: {
  ...
  mounted () {
    ...
    commonApi.getCodeIntType('entityType').then(res => {
      let viewList = res.data.data || []
      for (let entityTypeItem of viewList) {
        this.entityTypeCodeOptions.push({
          'label': entityTypeItem.codeDesc,
          'name': entityTypeItem.codeId
        })
      }
    }, res => {
      this.$message.error(res.data.message)
    })
    commonApi.getCodeIntType('pdstatus').then(res => {
      let viewList = res.data.data || []
      for (let pdstatusItem of viewList) {
        this.pdstatusCodeOptions.push({
          'label': pdstatusItem.codeDesc,
          'name': pdstatusItem.codeId
        })
      }
    }, res => {
      this.$message.error(res.data.message)
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailCoursePersonDynamic/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/CoursePersonDynamicTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailCoursePersonDynamic/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailCoursePersonDynamic/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
