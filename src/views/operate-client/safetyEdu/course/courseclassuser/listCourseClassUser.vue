            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.classUserId)"><i
...
<script>
import api from './courseclassuserUrl'
...
export default {
  data () {
    return {
      ...
      classUserStatusCodeOptions: [{'label': '全部', 'name': ''}],
      classUserStatusCodeValue: null,
      form: {},
      searchForm: {},
      data: '',
      multipleSelection: [],
  ...
  mounted () {
    ...
    commonApi.getCodeIntType('classUserStatus').then(res => {
      let viewList = res.data.data || []
      for (let classUserStatusItem of viewList) {
        this.classUserStatusCodeOptions.push({
          'label': classUserStatusItem.codeDesc,
          'name': classUserStatusItem.codeId
        })
      }
    }, res => {
  ...
  methods: {
    ...
    //删除
    toTotal () {
      let opath = '/safety/CourseClassUserTotal'
