      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="资源名称:" prop="name">
            <el-input class="input-v2-rao" v-model="searchForm.name" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="name" align="center" label="资源名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.name" class="one-line">{{scope.row.name}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="url" align="center" label="地址" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.url" class="one-line">{{scope.row.url}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="filePath" align="center" label="文件附件" sortable="custom">
              <template slot-scope="scope">
            ...
            <el-table-column prop="publishedDt" label="发布日期" align="center" sortable="custom"></el-table-column>
            ...
            <el-table-column prop="participateNum" align="center" label="学习人数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.participateNum" class="one-line">{{scope.row.participateNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailCourseResource/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/CourseResourceTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailCourseResource/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailCourseResource/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
