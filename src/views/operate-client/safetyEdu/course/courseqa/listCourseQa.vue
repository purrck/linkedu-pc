      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="课程:" prop="courseId">
            <el-input class="input-v2-rao" v-model="searchForm.courseId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="question" align="center" label="提问" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.question" class="one-line">{{scope.row.question}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="userName" align="center" label="提问者" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="askDt" label="提问时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="answer" align="center" label="答复" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answer" class="one-line">{{scope.row.answer}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="answerId" align="center" label="答复者" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerId" class="one-line">{{scope.row.answerId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="answerName" align="center" label="答复者" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerName" class="one-line">{{scope.row.answerName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="answerDt" label="答复时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="answerStatusCode" label="答复状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.answerStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.answerStatusCodeStr">{{scope.row.answerStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.qaId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.qaId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.qaId)"><i
...
export default {
  data () {
    return {
      ...
      answerStatusCodeOptions: [{'label': '全部', 'name': ''}],
      answerStatusCodeValue: null,
      form: {
        courseId: null,
      },
      searchForm: {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailCourseQa/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/CourseQaTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailCourseQa/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailCourseQa/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
