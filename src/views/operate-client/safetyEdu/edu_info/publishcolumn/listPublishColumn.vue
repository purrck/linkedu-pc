    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="资讯类别名称:" prop="columnName">
            <el-input clearable class="input-v2-rao" v-model="searchForm.columnName" maxlength="30" placeholder="请输入资讯类别名称"></el-input>
          </el-form-item>
          <el-form-item label="类别状态" prop="parentId">
            <el-select clearable class="select-v2-rao" v-model="searchForm.publicClassCode" placeholder="类别状态">
              <el-option
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="columnName" align="center" label="类别名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.columnName" class="one-line">{{scope.row.columnName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="childColumnNum" align="center" label="子类别数量" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.childColumnNum" class="one-line">{{scope.row.childColumnNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="articleNum" align="center" label="资讯文章数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.articleNum" class="one-line">{{scope.row.articleNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="publicClassCode" align="center" label="启用状态" sortable="custom">
              <template slot-scope="scope">
                <el-switch
                  v-model="scope.row.publicClassCode === '1'"
                  disabled
                  active-color="#93BF88"
                  inactive-color="#ccc">
                </el-switch>
            ...
            <!--<el-table-column prop="description" align="center" label="级别" sortable="custom">-->
            <!--<template slot-scope="scope">-->
            ...
            <el-table-column prop="createdBy" align="center" label="创建人" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.createdBy" class="one-line">{{scope.row.createdBy}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="createdDt" align="center" label="创建时间" sortable="custom">
              <template slot-scope="scope">
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.columnId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.columnId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.columnId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                  </el-popover>

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="子类别" trigger="hover">
                  <span slot="reference"
                        @click.stop="toChild(scope.row.columnId)"><i
                    class="iconfont icon-icon_fujian ml5 f20 c999999"></i></span>
                  </el-popover>
                </div>
              </template>
            </el-table-column>
...
export default {
  data () {
    return {
      ...
      publicClassCodeOptions: [{'label': '全部', 'name': ''}],
      publicClassCodeValue: null,
      defaultTemplateCodeOptions: [{'label': '全部', 'name': ''}],
      defaultTemplateCodeValue: null,
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailPublishColumnOp/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safetyEdu/PublishColumnTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailPublishColumnOp/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailPublishColumnOp/detail/' + id
      this.$router.push({path: opath})
    },
    toChild (id) {
      this.$router.push({path: '/safetyEdu/listPublishColumnChildOp', query: {columnId: id}})
    }
  }
}
</script>
