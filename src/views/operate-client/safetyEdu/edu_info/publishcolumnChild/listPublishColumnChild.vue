      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="类别状态" prop="parentId">
            <el-select clearable v-model="searchForm.publicClassCode" placeholder="请选择类别状态">
              <el-option
      ...
      <el-main class="pt-15">
        <el-row class="mb10">
          <el-col :span="24" class="text-left mb-10 flex row btns x-center space-between">
            ...
            <el-button type="primary" size="small" class="el-button--small-icon  button-v2-rao b93BF88 " @click= "toAdd">
              <i class="iconfont icon-icon_xinjian f12 cfff"></i> 新增子类别
            </el-button>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="columnName" align="center" label="子类别名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.columnName" class="one-line">{{scope.row.columnName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="parentColumnName" align="center" label="类别名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.parentColumnName" class="one-line">{{scope.row.parentColumnName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = `/safetyEdu/detailPublishColumnChildOp?type=add&columnId=${this.columnId}`
    ...
    toEdit (id) {
      ...
      let opath = `/safetyEdu/detailPublishColumnChildOp?type=edit&columnId=${this.columnId}&childColumnId=${id}`
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = `/safetyEdu/detailPublishColumnChildOp?type=detail&columnId=${this.columnId}&childColumnId=${id}`
      this.$router.push({path: opath})
    },
  }
}
