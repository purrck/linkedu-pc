      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="othersInd" align="center" label="其他答案" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.othersInd" class="one-line">{{scope.row.othersInd}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="othersLabel" align="center" label="其他答案标题" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.othersLabel" class="one-line">{{scope.row.othersLabel}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="layoutCd" align="center" label="显示" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.layoutCd" class="one-line">{{scope.row.layoutCd}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.surveyIdsequence)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.surveyIdsequence)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.surveyIdsequence)"><i
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailSurveyQnChoice/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SurveyQnChoiceTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailSurveyQnChoice/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailSurveyQnChoice/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
