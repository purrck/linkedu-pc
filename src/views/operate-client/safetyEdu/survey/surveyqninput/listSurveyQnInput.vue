      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="labelTextId" align="center" label="问题输入" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.labelTextId" class="one-line">{{scope.row.labelTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="width" align="center" label="长度" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.width" class="one-line">{{scope.row.width}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="maxLinesChars" align="center" label="最多输入字数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.maxLinesChars" class="one-line">{{scope.row.maxLinesChars}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.labelTextId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.labelTextId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.labelTextId)"><i
...
<script>
import api from './surveyqninputUrl'
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailSurveyQnInput/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safetyEdu/SurveyQnInputTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailSurveyQnInput/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailSurveyQnInput/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
