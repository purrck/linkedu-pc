      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="answerStatus" align="center" label="提交状态" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerStatus" class="one-line">{{scope.row.answerStatus}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="firstName" align="center" label="名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.firstName" class="one-line">{{scope.row.firstName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailSurveyAnsMaster/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SurveyAnsMasterTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailSurveyAnsMaster/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailSurveyAnsMaster/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
