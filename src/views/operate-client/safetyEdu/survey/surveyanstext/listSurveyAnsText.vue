      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="answerTextId" align="center" label="问卷答案信息" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerTextId" class="one-line">{{scope.row.answerTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="answerTextContent" align="center" label="问卷答案内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerTextContent" class="one-line">{{scope.row.answerTextContent}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.answerTextId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.answerTextId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.answerTextId)"><i
...
<script>
import api from './surveyanstextUrl'
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailSurveyAnsText/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SurveyAnsTextTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailSurveyAnsText/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailSurveyAnsText/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
