<template>
  <div class="SurveyQnChoiceOption">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="optionLabel" align="center" label="选项" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.optionLabel" class="one-line">{{scope.row.optionLabel}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.optionId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.optionId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.optionId)"><i
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailSurveyQnChoiceOption/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SurveyQnChoiceOptionTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailSurveyQnChoiceOption/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailSurveyQnChoiceOption/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
