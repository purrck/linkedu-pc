      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="sequence" align="center" label="序号" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.sequence" class="one-line">{{scope.row.sequence}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="qnType" align="center" label="问题类型" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.qnType" class="one-line">{{scope.row.qnType}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="qnChoice" align="center" label="问题选项" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.qnChoice" class="one-line">{{scope.row.qnChoice}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="answerTextId" align="center" label="回复文字" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerTextId" class="one-line">{{scope.row.answerTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.surveyIdanswerIdsequence)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.surveyIdanswerIdsequence)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.surveyIdanswerIdsequence)"><i
...
<script>
import api from './surveyansdetailUrl'
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailSurveyAnsDetail/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SurveyAnsDetailTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailSurveyAnsDetail/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailSurveyAnsDetail/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
