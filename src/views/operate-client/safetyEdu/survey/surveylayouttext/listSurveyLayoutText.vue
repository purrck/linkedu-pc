      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="textContent" align="center" label="文字信息内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.textContent" class="one-line">{{scope.row.textContent}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.textId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.textId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.textId)"><i
...
<script>
import api from './surveylayouttextUrl'
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailSurveyLayoutText/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safetyEdu/SurveyLayoutTextTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailSurveyLayoutText/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailSurveyLayoutText/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
