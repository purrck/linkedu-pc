      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="问卷:" prop="surveyId">
            <el-input class="input-v2-rao" v-model="searchForm.surveyId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="headerTextId" align="center" label="问卷头部内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.headerTextId" class="one-line">{{scope.row.headerTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="footerTextId" align="center" label="问卷底部内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.footerTextId" class="one-line">{{scope.row.footerTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="exitMessageTextId" align="center" label="问卷提交显示内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.exitMessageTextId" class="one-line">{{scope.row.exitMessageTextId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="bgRgb" align="center" label="背景色" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.bgRgb" class="one-line">{{scope.row.bgRgb}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="fgRgb" align="center" label="字体颜色" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.fgRgb" class="one-line">{{scope.row.fgRgb}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="fontStyle" align="center" label="文字风格" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.fontStyle" class="one-line">{{scope.row.fontStyle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
...
<script>
import api from './surveylayoutUrl'
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safetyEdu/detailSurveyLayout/add/' + 0
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safetyEdu/SurveyLayoutTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safetyEdu/detailSurveyLayout/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safetyEdu/detailSurveyLayout/detail/' + id
      this.$router.push({path: opath})
    },
  }
}
