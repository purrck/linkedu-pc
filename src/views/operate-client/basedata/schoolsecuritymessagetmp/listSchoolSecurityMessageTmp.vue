...
<template>
  <div class="SchoolSecurityMessageTmp">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="标题:"  prop="messageTmpTitle" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.messageTmpTitle" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="提醒内容:"  prop="messageTmpContent" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.messageTmpContent" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="内容状态:" prop="messageTmpStatusCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.messageTmpStatusCode" placeholder="请您选择">
                ...
                <el-form-item label="提醒适合类型:" prop="messageTmpTypeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.messageTmpTypeCode" placeholder="请您选择">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="messageTmpTitle" align="center" label="标题" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.messageTmpTitle" class="one-line">{{scope.row.messageTmpTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="messageTmpStatusCode" label="内容状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.messageTmpStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.messageTmpStatusCodeStr">{{scope.row.messageTmpStatusCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 内容状态 colName:message_tmp_status_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="messageTmpTypeCode" label="提醒适合类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.messageTmpTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.messageTmpTypeCodeStr">{{scope.row.messageTmpTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
...
<!-- 发布时间 colName:published_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="usedTimesNum" align="center" label="被使用" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.usedTimesNum" class="one-line">{{scope.row.usedTimesNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 被使用 colName:used_times_num sqlType:INTEGER notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.messageTmpId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.messageTmpId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.messageTmpId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form : {
          messageTmpTitle :null,
          messageTmpContent :null,
          messageTmpStatusCodeValue: null,
          messageTmpTypeCodeValue: null,
        },
        searchForm : {
          messageTmpTitle :null,
          messageTmpContent :null,
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolSecurityMessageTmp/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolSecurityMessageTmpTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolSecurityMessageTmp/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolSecurityMessageTmp/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
