    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="学校日常作息:" prop="timelineName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.timelineName" maxlength="30"></el-input>
          </el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="timelineName" label="作息名称" align="center"></el-table-column>
            <el-table-column prop="schoolOrgName" align="center" label="学校机构" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.schoolOrgName" class="one-line">{{scope.row.schoolOrgName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="schoolTerm" align="center" label="学期" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.schoolTermStr" class="one-line">{{scope.row.schoolTermStr.split(';')[0]}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="remark" align="center" label="备注" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.remark" class="one-line" v-html="scope.row.remark"></div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right" width="180">
              <template slot-scope="scope">
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.timelineId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.timelineId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.timelineId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                  </el-popover>

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="学校日常作息安排" trigger="hover">
                      <span slot="reference"
                            @click.stop="toAnother(scope.row ,`listOrgSchoolTimelineDetail`)"><i
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/basedata/detailOrgSchoolTimeline/add/' + 0;
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/basedata/OrgSchoolTimelineTotal';
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/basedata/detailOrgSchoolTimeline/edit/' + id;
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailOrgSchoolTimeline/detail/' + id;
      this.$router.push({path: opath})
    },
    toAnother (item, path) {
      let opath = '/basedata/' + path + '?timelineId=' + item.timelineId+'&timelineName='+item.timelineName;
      this.$router.push({path: opath})
    },
  }
}
