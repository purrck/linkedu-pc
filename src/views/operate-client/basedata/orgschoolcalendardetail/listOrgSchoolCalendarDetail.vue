      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="学校机构:" prop="schoolOrgId">
            <el-input class="input-v2-rao" v-model="searchForm.schoolOrgId" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="安排类型:" prop="calendarTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.calendarTypeCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="calendarTypeCode" label="安排类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.calendarTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.calendarTypeCodeStr">{{scope.row.calendarTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="calendarDt" label="日期(年月日)" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="isOverdue" label="过期" align="center" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isOverdue == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.calendarDetailId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.calendarDetailId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.calendarDetailId)"><i
...
<script>
  ...
  export default {
    ...
    created() {
      let calendarId = this.$route.query.calendarId;
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/basedata/detailOrgSchoolCalendarDetail/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/basedata/OrgSchoolCalendarDetailTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailOrgSchoolCalendarDetail/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailOrgSchoolCalendarDetail/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
