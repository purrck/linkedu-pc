<template>
  <div class="OrgSchoolTimelineDetail">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="作息详细名称:" prop="timelineId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.timelineDetailName" maxlength="30"></el-input>
          </el-form-item>

          <el-form-item label="作息类型:" prop="timelineTypeCode" class="w140">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="timelineDetailName" align="center" label="作息详细名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.timelineDetailName" class="one-line">{{scope.row.timelineDetailName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>            <el-table-column prop="timelineName" align="center" label="学校日常作息" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.timelineName" class="one-line">{{scope.row.timelineName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="endTime" align="center" label="结束时间" sortable="custom">
              <template slot-scope="scope">
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <div class="flex  row space-between" style="padding: 0 20px">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.timelineDetailId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.timelineDetailId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.timelineDetailId)"><i
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.params.timelineId = timelineId;
    },
    mounted() {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = `/basedata/detailOrgSchoolTimelineDetail/add/0?timelineId=${this.$route.query.timelineId}&timelineName=${this.$route.query.timelineName}`;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/basedata/OrgSchoolTimelineDetailTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailOrgSchoolTimelineDetail/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailOrgSchoolTimelineDetail/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
