    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          <el-form-item label="机构部门模板:">
            <el-input v-model="searchForm.orgDepartTempId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="机构部门模板:">
            <el-input v-model="searchForm.orgDepartTempName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="机构类型:">
            <el-select v-model="searchForm.orgTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="部门模板状态:">
            <el-select v-model="searchForm.departTempStatusCodeValue" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="orgDepartTempName" label="机构部门模板" sortable="custom" width="200"></el-table-column>
            <el-table-column label="机构类型">
              <template slot-scope="scope">
                <span :class="scope.row.orgTypeCode" :style="{color:scope.row.orgTypeCodeStr.split(';')[1]}">{{scope.row.orgTypeCodeStr.split(';')[0]}}</span>
              </template>
            </el-table-column>
            <!--<el-table-column prop="effectiveDt" label="生效时间" sortable="custom"-->
            <!--width="200"></el-table-column>-->
            <!--<el-table-column prop="expiryDt" label="失效时间" sortable="custom"-->
            <!--width="200"></el-table-column>-->
            <!--<el-table-column  prop="isCurrent" label="当前应用" sortable="custom" >-->
            <!--<template slot-scope="scope">-->
            <!--<div v-if="scope.row.isCurrent == 'Y'">-->
            <!--<el-switch v-model="onSwitchStatus" disabled></el-switch>-->
            <!--</div>-->
            <!--<div v-else>-->
            <!--<el-switch v-model="offSwitchStatus" disabled></el-switch>-->
            <!--</div>-->
            <!--</template>-->
            <!--</el-table-column>-->
            <!--<el-table-column label="部门模板状态">-->
            <!--<template slot-scope="scope">-->
            <!--<span :class="scope.row.departTempStatusCode"  :style="{color:scope.row.departTempStatusCodeStr.split(';')[1]}" >{{scope.row.departTempStatusCodeStr.split(';')[0]}}</span>-->
            <!--</template>-->
            <!--</el-table-column>-->
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.orgDepartTempId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                  <span slot="reference"
                        @click.stop="toEdit(scope.row.orgDepartTempId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.orgDepartTempId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>
<!--                -->
<!--                <el-button size="small" type="primary"-->
<!--                           class="tableEditButton"-->
<!--                           @click.stop="toEdit(scope.row.orgDepartTempId)">-->
<!--                  编辑-->
<!--                </el-button>-->
<!--                <el-button size="small" type="primary"-->
<!--                           @click="toDetail(scope.row.orgDepartTempId)">-->
<!--                  详情-->
<!--                </el-button>-->
<!--                <el-button size="small" type="danger"-->
<!--                           @click="rowDel(scope.row.orgDepartTempId)">删除-->
<!--                </el-button>-->
                <!--<el-button size="small" type="primary"  @click="toDetail(scope.row.orgDepartTempId)">-->
                <!--模板部门-->
                <!--</el-button>-->
              </template>
            </el-table-column>
  ...
  export default {
    data() {
      return {
        ...
        orgTypeCodeOptions: [{"label": "全部", "name": ""}],
        orgTypeCodeValue: null,
        ynOptions: [{"label": "全部", "name": ""}],
        ynValue: null,
        departTempStatusCodeOptions: [{"label": "全部", "name": ""}],
        departTempStatusCodeValue: null,
        form: {
          orgDepartTempId:
            null,
          orgDepartTempName:
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/basedata/detailDepartTemplate/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailDepartTemplate/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailDepartTemplate/detail/' + id;
        this.$router.push({path: opath})
      },
      getorgTypeCodeStr(row) {
        return row.orgTypeCodeStr;
      },
      geteffectiveDtStr(row) {
        return row.effectiveDtStr;
      },
      getexpiryDtStr(row) {
        return row.expiryDtStr;
      },
      getisCurrentStr(row) {
        return row.isCurrentStr;
      },
      getdepartTempStatusCodeStr(row) {
        return row.departTempStatusCodeStr;
      },
    }
  }
</script>
