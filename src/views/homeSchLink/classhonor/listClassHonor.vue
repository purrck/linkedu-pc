      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="荣誉:" prop="honorTitle">
            <el-input class="input-v2-rao" v-model="searchForm.honorTitle" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="获取时间:" prop="gotDt">
            <el-date-picker class="date-v2-rao" v-model="searchForm.gotDt" type="date"
                            value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
          </el-form-item>
          <el-form-item label="荣誉类别:" prop="honorTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.honorTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="荣誉级别:" prop="honorLevelCode">
            <el-select class="select-v2-rao" v-model="searchForm.honorLevelCode" placeholder="请您选择">
          ...
          <el-form-item label="状态:" prop="honorStatusCode">
            <el-select class="select-v2-rao" v-model="searchForm.honorStatusCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="studentUserPreviewVO" align="center" label="学生" >
            ...
            <el-table-column prop="honorTitle" align="center" label="荣誉" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.honorTitle" class="one-line">{{scope.row.honorTitle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="gotDt" label="获取时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="honorTypeCode" label="荣誉类别" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.honorTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.honorTypeCodeStr">{{scope.row.honorTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="honorLevelCode" label="荣誉级别" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.honorLevelCodeStr.split(';')[1]}"
                                  v-if="scope.row.honorLevelCodeStr">{{scope.row.honorLevelCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.classHonorId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.classHonorId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.classHonorId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        honorTypeCodeOptions: [{"label": "全部", "name": ""}],
        honorTypeCodeValue: null,
        honorLevelCodeOptions: [{"label": "全部", "name": ""}],
        honorLevelCodeValue: null,
        honorStatusCodeOptions: [{"label": "全部", "name": ""}],
        honorStatusCodeValue: null,
        form: {
          honorTitle: null,
          gotDt: null,
          honorTypeCodeValue: null,
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/homeSchLink/detailClassHonor/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/homeSchLink/ClassHonorTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/homeSchLink/detailClassHonor/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/homeSchLink/detailClassHonor/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
