      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="头像:" prop="imagePath">
            <el-input class="input-v2-rao" v-model="searchForm.imagePath" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="类型:" prop="pacesetterTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.pacesetterTypeCode" placeholder="请您选择">
              <el-option v-for="pacesetterTypeCodeItem in pacesetterTypeCodeOptions"
                         :key="pacesetterTypeCodeItem.name"
                         :label="pacesetterTypeCodeItem.label" :value="pacesetterTypeCodeItem.name">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <!--<el-table-column prop="imagePath" align="center" label="头像">-->
              ...
              <!--</template>-->
            <!--</el-table-column>-->
            <el-table-column prop="pacesetterTypeCode" label="类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.pacesetterTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.pacesetterTypeCodeStr">{{scope.row.pacesetterTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.classPacesetterId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.classPacesetterId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.classPacesetterId)"><i
...
<script>
  import api from "./classpacesetterUrl";
  ...
  export default {
    ...
    data() {
      return {
        ...
        pacesetterTypeCodeOptions: [{"label": "全部", "name": ""}],
        pacesetterTypeCodeValue: null,
        form: {
          imagePath: null,
          pacesetterTypeCodeValue: null,
        },
    ...
    mounted() {
      ...
      commonApi.getCodeIntType("pacesetterType").then(res => {
        let viewList = res.data.data || [];
        for (let pacesetterTypeItem of viewList) {
          this.pacesetterTypeCodeOptions.push({
            "label": pacesetterTypeItem.codeDesc,
            "name": pacesetterTypeItem.codeId
          });
        }
      }, res => {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/homeSchLink/detailClassPacesetter/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/homeSchLink/ClassPacesetterTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/homeSchLink/detailClassPacesetter/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/homeSchLink/detailClassPacesetter/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
