      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="作品名称:" prop="workTitle">
            <el-input class="input-v2-rao" v-model="searchForm.workTitle" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="作品类别:" prop="workTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.workTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="状态:" prop="workStatusCode">
            <el-select class="select-v2-rao" v-model="searchForm.workStatusCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="workTitle" align="center" label="作品名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.workTitle" class="one-line">{{scope.row.workTitle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="imagePath" align="center" label="作品图">
              <template slot-scope="scope">
            ...
            <el-table-column prop="workDt" label="发布时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="workTypeCode" label="作品类别" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.workTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.workTypeCodeStr">{{scope.row.workTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="workStatusCode" label="状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.workStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.workStatusCodeStr">{{scope.row.workStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.bestWorkId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.bestWorkId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.bestWorkId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        workTypeCodeOptions: [{"label": "全部", "name": ""}],
        workTypeCodeValue: null,
        workStatusCodeOptions: [{"label": "全部", "name": ""}],
        workStatusCodeValue: null,
        form: {
          workTitle: null,
          workTypeCodeValue: null,
          workStatusCodeValue: null,
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/homeSchLink/detailClassBestWork/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/homeSchLink/ClassBestWorkTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/homeSchLink/detailClassBestWork/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/homeSchLink/detailClassBestWork/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
