<template>
  <div class="ClassCommitteeMember">
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="班委类型:" prop="classCommitteeCode">
            <el-select class="select-v2-rao" v-model="searchForm.classCommitteeCode" placeholder="请您选择">
          ...
          <el-form-item label="适合班级:" prop="classId">
            <el-input class="input-v2-rao" v-model="searchForm.classId" maxlength="30"></el-input>
          </el-form-item>

          <el-form-item label="状态:" prop="committeeMemberStatusCode">
            <el-select class="select-v2-rao" v-model="searchForm.committeeMemberStatusCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="classCommitteeCode" label="班委类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.classCommitteeCodeStr.split(';')[1]}"
                                  v-if="scope.row.classCommitteeCodeStr">{{scope.row.classCommitteeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="committeeMemberStatusCode" label="状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.committeeMemberStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.committeeMemberStatusCodeStr">{{scope.row.committeeMemberStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.committeeMemberId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.committeeMemberId)"><i
                  ...
                  <!--<span slot="reference"-->
                        <!--@click.stop="rowDel(scope.row.committeeMemberId)"><i-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        classCommitteeCodeOptions: [{"label": "全部", "name": ""}],
        classCommitteeCodeValue: null,
        committeeMemberStatusCodeOptions: [{"label": "全部", "name": ""}],
        committeeMemberStatusCodeValue: null,
        form: {
          classCommitteeCodeValue: null,
          committeeMemberStatusCodeValue: null,
        },
        searchForm: {
          classCommitteeCode: null,
          committeeMemberStatusCode: null,
        },
        data: "",
        multipleSelection: [],
        params: {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/homeSchLink/detailClassCommitteeMember/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/homeSchLink/ClassCommitteeMemberTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/homeSchLink/detailClassCommitteeMember/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/homeSchLink/detailClassCommitteeMember/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
