<template>
  <div class="StudentFamilyVisitResult">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="家访id:" prop="familyVisitId" class="w140">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="userName" align="center" label="记录人" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="resultContent" align="center" label="记录内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.resultContent" class="one-line">{{scope.row.resultContent}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.familyVisitResultId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.familyVisitResultId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.familyVisitResultId)"><i
...
<script>
  import api from "./studentfamilyvisitresultUrl";
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        this.multipleSelection = val.map(item => {
          return item.familyVisitResultId
        });
      },
      ...
      toAdd() {
        ...
        let opath = '/homeSchLink/detailStudentFamilyVisitResult/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/homeSchLink/StudentFamilyVisitResultTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/homeSchLink/detailStudentFamilyVisitResult/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/homeSchLink/detailStudentFamilyVisitResult/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
