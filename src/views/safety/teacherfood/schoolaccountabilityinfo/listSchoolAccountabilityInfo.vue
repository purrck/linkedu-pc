<template>
  <div class="SchoolAccountabilityInfo">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="责任追究事件:" prop="accountabilityInfoId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.accountabilityInfoId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="事件:" prop="accountabilityInfoTitle" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.accountabilityInfoTitle" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="accountabilityInfoTitle" align="center" label="事件" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.accountabilityInfoTitle" class="one-line">{{scope.row.accountabilityInfoTitle}}
                </div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="filePath" align="center" label="事件报告" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.filePath" class="one-line">{{scope.row.filePath}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="accountabilityRuleTitle" align="center" label="适用追究规则" sortable="custom">
              <template slot-scope="scope">
            ...
            <el-table-column prop="punishAmount" align="center" label="处罚金额" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.punishAmount" class="one-line">{{scope.row.punishAmount}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="accountabilityInfoStatusCode" label="事件状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.accountabilityInfoStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.accountabilityInfoStatusCodeStr">{{scope.row.accountabilityInfoStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.accountabilityInfoId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.accountabilityInfoId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.accountabilityInfoId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>

                <el-popover placement="top-start" popper-class="popover-v2-rao" title="责任追究干系人员" trigger="hover">
                      <span slot="reference"
                            @click.stop="toAnother(scope.row.accountabilityInfoId ,`listSchoolAccountabilityPerson`)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form: {
          accountabilityInfoId: null,
          accountabilityInfoTitle: null,
        },
        searchForm: {
          accountabilityInfoId: null,
          accountabilityInfoTitle: null,
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        this.multipleSelection = val.map(item => {
          return item.accountabilityInfoId
        });
      },
      ...
      toAdd() {
        ...
        let opath = '/safety/detailSchoolAccountabilityInfo/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/SchoolAccountabilityInfoTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailSchoolAccountabilityInfo/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailSchoolAccountabilityInfo/detail/' + id;
        this.$router.push({path: opath})
      },
      toAnother(id, path) {
        let opath = '/safety/' + path + '?accountabilityInfoId=' + id;
        this.$router.push({path: opath})
      },
    }
  }
