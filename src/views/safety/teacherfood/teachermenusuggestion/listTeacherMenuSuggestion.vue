<template>
  <div class="TeacherMenuSuggestion">
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="建议:" prop="suggestionTitle">
            <el-input class="input-v2-rao" v-model="searchForm.suggestionTitle" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="建议时间:" prop="suggestedDt">
            <el-date-picker class="date-v2-rao" v-model="searchForm.suggestedDt" type="date"
          ...
          <el-form-item label="教师类型:" prop="teacherTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.teacherTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="教师职位:" prop="teacherPositionName">
            <el-input class="input-v2-rao" v-model="searchForm.teacherPositionName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="suggestionTitle" align="center" label="建议" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.suggestionTitle" class="one-line">{{scope.row.suggestionTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
                  <el-table-column prop="imagePath" align="center" label="图片">
                    <template slot-scope="scope">
                      <div v-if="scope.row.imagePath">
                        <img :src="scope.row.imagePath" alt="" height="60px" class="scope-img">
                      </div>
                      <div v-if="!scope.row.imagePath">无</div>
                    </template>
                  </el-table-column>
                  <el-table-column prop="suggestedDt" label="建议时间" align="center" sortable="custom"></el-table-column>
                  <el-table-column prop="menuSuggestionStatusCode" label="建议状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.menuSuggestionStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.menuSuggestionStatusCodeStr">{{scope.row.menuSuggestionStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="teacherTypeCode" label="教师类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.teacherTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.teacherTypeCodeStr">{{scope.row.teacherTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="teacherPositionName" align="center" label="教师职位" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.teacherPositionName" class="one-line">{{scope.row.teacherPositionName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.menuSuggestionId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.menuSuggestionId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.menuSuggestionId)"><i
  ...
  export default {
    data() {
      return {
        ...
        menuSuggestionStatusCodeOptions: [{"label": "全部", "name": ""}],
        menuSuggestionStatusCodeValue: null,
        teacherTypeCodeOptions: [{"label": "全部", "name": ""}],
        teacherTypeCodeValue: null,
        form: {
          suggestionTitle: null,
          suggestedDt: null,
          teacherName: null,
          teacherTypeCodeValue: null,
          teacherPositionName: null,
        },
        searchForm: {
          suggestionTitle: null,
          suggestedDt: null,
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/safety/detailTeacherMenuSuggestion/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/TeacherMenuSuggestionTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
                                let opath = '/safety/detailTeacherMenuSuggestion/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailTeacherMenuSuggestion/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
