      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="phoneNum" label="手机号" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="effectiveDt" label="开启时间" sortable="custom"
                             width="200"></el-table-column>
            <el-table-column prop="expiryDt" label="失效时间" sortable="custom"
                             width="200"></el-table-column>
            <el-table-column label="手机号状态">
              <template slot-scope="scope">
                <span :class="scope.row.phonenumStatusCode"
                      :style="{color:scope.row.phonenumStatusCodeStr.split(';')[1]}">{{scope.row.phonenumStatusCodeStr.split(';')[0]}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="lastLoginDt" label="上次登录时间" sortable="custom"
                             width="200"></el-table-column>
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           class="tableEditButton"
                           @click.stop="toEdit(scope.row.phonenumId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.phonenumId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.phonenumId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
...
<script>
  import api from "./aasubjectphonenumUrl";
  ...
  export default {
    data() {
      return {
        ...
        phonenumStatusCodeOptions: [{"label": "全部", "name": ""}],
        phonenumStatusCodeValue: null,
        form: {},
        searchForm: {}
        ,
        data: "",
    ...
    mounted() {
      ...
      commonApi.getCodeIntType("phonenumStatus").then(res => {
        let viewList = res.data.data || [];
        for (let phonenumStatusItem of viewList) {
          this.phonenumStatusCodeOptions.push({
            "label": phonenumStatusItem.codeDesc,
            "name": phonenumStatusItem.codeId
          });
        }
      }, res => {
   ...
   this.params.page = currentPage
      ...
      toAdd() {
        let opath = '/safety/detailAaSubjectPhoneNum/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailAaSubjectPhoneNum/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailAaSubjectPhoneNum/detail/' + id;
        this.$router.push({path: opath})
      },
      ...
      getphonenumStatusCodeStr(row) {
        return row.phonenumStatusCodeStr;
      },
      getlastLoginDtStr(row) {
        return row.lastLoginDtStr;
      },
    }
  }
</script>
