<template>
  <div class="SecurityAccidentProcess">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="安全事故:" prop="accidentId">
            <el-input class="input-v2-rao" v-model="searchForm.accidentId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.accidentProcessId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.accidentProcessId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.accidentProcessId)"><i
...
export default {
  ...
  methods: {
    finish(){
      ...
      api.securityAccidentAdFinish({
        accidentId : this.$route.query.accidentId
      }).then(res => {
        if (res.data.resultCode == '000000') {
          this.$message.success('操作成功')
          let opath = `/safety/listSecurityAccident`
    ...
    toAdd () {
      ...
      let opath = '/safety/detailSecurityAccidentProcessadd/' + 0 + `?accidentId=${this.$route.query.accidentId}&accidentTitle=${this.$route.query.accidentTitle}`
      this.$router.push({path: opath})
    },
    toTotal () {
      let opath = '/safety/SecurityAccidentProcessTotal'
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safety/detailSecurityAccidentProcessedit/' + id + `?accidentId=${this.$route.query.accidentId}&accidentTitle=${this.$route.query.accidentTitle}`
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safety/detailSecurityAccidentProcessdetail/' + id
      this.$router.push({path: opath})
    },
  }
}
