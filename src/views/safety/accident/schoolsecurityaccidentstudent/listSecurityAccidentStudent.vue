<template>
  <div class="SecurityAccidentStudent">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.accidentStudentId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.accidentStudentId)"><i
      ...
      <el-dialog
        ...
        <div style="display: flex;">
          <el-input v-model="studentName" placeholder="学生名字"></el-input>
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="性别">
            <template slot-scope="scope">
              <span>{{scope.row.gender === 'M' ? '女': '男'}}</span>
            </template>
          </el-table-column>
        </el-table>
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible = false">取 消</el-button>
          <el-button type="primary" :loading="addStudentLoading" @click="addStudent">确 定</el-button>
        </span>
      </el-dialog>
    </el-container>
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      // let opath = '/safety/detailSecurityAccidentStudentV2/add/' + 0 + `?accidentId=${this.$route.query.accidentId}&accidentTitle=${this.$route.query.accidentTitle}`
      // this.$router.push({path: opath})
      this.dialogVisible = true
      this.searchStudent()
    },
    toTotal () {
      let opath = '/safety/SecurityAccidentStudentTotal'
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safety/detailSecurityAccidentStudentV2/detail/' + id
      this.$router.push({path: opath})
    },
    addStudent () {
      this.dialogVisible = false
      let arr = this.multipleStudentSeletion.map(item => {
        return {
          studentName: item.userName,
          studentUserId: item.userId,
          studentId: item.studentId,
          accidentId: this.$route.query.accidentId,
          schoolId : this.schoolInfo.schoolOrgId
        }
      })
      this.addStudentLoading = true;
      api.addStudentListAddBatch(arr).then(res => {
        if(res.data.resultCode !== "000000"){
