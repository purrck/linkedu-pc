        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          ...
          <el-form-item label="需要监控:">
            <el-select v-model="searchForm.ynValue" placeholder="请您选择">
              <el-option v-for="isNeedAccItem in ynOptions"
                         :key="isNeedAccItem.name"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="uri" label="uri" sortable="custom" width="300"></el-table-column>
            <el-table-column prop="name" label="名称" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="isNeedAcc" label="需要监控" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isNeedAcc == 'Y'" disabled></el-switch>
                </div>
            ...
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.uri)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.uri)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.uri)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    data() {
      return {
        ...
        ynValue: null,
        uriTypeCodeOptions: [{"label": "全部", "name": ""}],
        uriTypeCodeValue: null,
        form: {
          name:
            null,
          ynValue: null,
   ...
   this.params.page = currentPage
      ...
      toAdd() {
        let opath = '/safety/detailAccessmetricsUri/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailAccessmetricsUri/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailAccessmetricsUri/detail/' + 11;
        this.$router.push({path: opath})
      },
      getisNeedAccStr(row) {
        return row.isNeedAccStr;
      },
    }
  }
</script>
