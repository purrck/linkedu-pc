<template>
	<div class="SecurityOrganization">
...
<!--					<el-form-item label="id:" prop="securityOrganizationId">-->
<!--						<el-input class="input-v2-rao" v-model="searchForm.securityOrganizationId" maxlength="30"></el-input>-->
<!--					</el-form-item>-->
					<el-form-item label="安防名称:" prop="securityOrganizationName">
						<el-input class="input-v2-rao" v-model="searchForm.securityOrganizationName" maxlength="30"></el-input>
					</el-form-item>
					<el-form-item label="安防类型:" prop="securityOrganizationTypeCode">
						<el-select class="select-v2-rao" v-model="searchForm.securityOrganizationTypeCode" placeholder="请您选择">
							<el-option v-for="securityOrganizationTypeCodeItem in securityOrganizationTypeCodeOptions" :key="securityOrganizationTypeCodeItem.name" :label="securityOrganizationTypeCodeItem.label" :value="securityOrganizationTypeCodeItem.name">
							</el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="地址:" prop="organizationAddressGps">
						<el-input class="input-v2-rao" v-model="searchForm.organizationAddressGps" maxlength="30"></el-input>
					</el-form-item>
					<el-form-item label="电话:" prop="organizationPhonenum">
						<el-input class="input-v2-rao" v-model="searchForm.organizationPhonenum" maxlength="30"></el-input>
					</el-form-item>
					<el-form-item label="更新时间:" prop="refreshedDt">
						<el-date-picker class="date-v2-rao" v-model="searchForm.refreshedDt" type="date" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
					</el-form-item>
					<el-form-item>
			...
			<el-main class="pt-15">
				...
				<div class="table">
					<el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border @selection-change="handleSelectionChange" class="table-v2-rao" ref="schooltable" header-row-class-name="table-header-v2-rao" :stripe="true" @sort-change="sortChange">
						...
						<el-table-column prop="securityOrganizationName" align="center" label="安防名称" sortable="custom">
							<template slot-scope="scope">
								<div v-if="scope.row.securityOrganizationName" class="one-line">{{scope.row.securityOrganizationName}}</div>
								<div v-else>无</div>
							</template>
						</el-table-column>
						<el-table-column prop="securityOrganizationTypeCode" label="安防类型" align="center" sortable="custom">
							<template slot-scope="scope">
								<span :style="{color:scope.row.securityOrganizationTypeCodeStr.split(';')[1]}" v-if="scope.row.securityOrganizationTypeCodeStr">{{scope.row.securityOrganizationTypeCodeStr.split(';')[0]}}</span>
								<span v-else>无</span>
							</template>
						</el-table-column>
						...
						<el-table-column prop="organizationAddressGps" align="center" label="地址" sortable="custom">
							<template slot-scope="scope">
								<div v-if="scope.row.organizationAddressGps" class="one-line">{{scope.row.organizationAddressGps}}</div>
								<div v-else>无</div>
							</template>
						</el-table-column>
						<el-table-column prop="organizationPhonenum" align="center" label="电话" sortable="custom">
							<template slot-scope="scope">
								<div v-if="scope.row.organizationPhonenum" class="one-line">{{scope.row.organizationPhonenum}}</div>
								<div v-else>无</div>
							</template>
						</el-table-column>
						<el-table-column prop="refreshedDt" label="更新时间" align="center" sortable="custom"></el-table-column>
							...
							<template slot-scope="scope">
								<div class="flex  row">
									...
									<el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
										<span slot="reference" @click.stop="toDetail(scope.row.securityOrganizationId)"><i
                        class="iconfont icon-icon_chakan1 ml5 f20 c999999"></i></span>
									...
									<el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
										<span slot="reference" @click.stop="rowDel(scope.row.securityOrganizationId)"><i
                      class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
									</el-popover>

									<el-popover placement="top-start" popper-class="popover-v2-rao" title="安全联防联系人" trigger="hover">
										<span slot="reference" @click.stop="toAnother(scope.row.securityOrganizationId ,`listSecurityOrganizationContact`)"><i
	...
	export default {
		data() {
			return {
				...
				securityOrganizationTypeCodeOptions: [{
					"label": "全部",
					"name": ""
				}],
				securityOrganizationTypeCodeValue: null,
				form: {
					securityOrganizationId: null,
					securityOrganizationName: null,
					securityOrganizationTypeCodeValue: null,
					organizationAddressGps: null,
					organizationPhonenum: null,
					refreshedDt: null,
				},
				searchForm: {
					securityOrganizationId: null,
					securityOrganizationName: null,
					securityOrganizationTypeCode: null,
					organizationAddressGps: null,
					organizationPhonenum: null,
					refreshedDt: null,
				},
		...
		methods: {
			...
			handleSelectionChange(val) {
				this.multipleSelection = val.map(item => {
					return item.securityOrganizationId
				});
			},
			...
			toAdd() {
				...
				let opath = '/safety/detailSecurityOrganization/add/' + 0;
				this.$router.push({
					path: opath
				})
			},
			toTotal() {
				let opath = '/safety/SecurityOrganizationTotal';
				this.$router.push({
					path: opath
				})
			},
			toEdit(id) {
				let opath = '/safety/detailSecurityOrganization/edit/' + id;
				this.$router.push({
					path: opath
				})
			},
			toDetail(id) {
				let opath = '/safety/detailSecurityOrganization/detail/' + id;
				this.$router.push({
					path: opath
				})
			},
			toAnother(id, path) {
				let opath = '/safety/' + path + '?securityOrganizationId=' + id;
				this.$router.push({
					path: opath
				})
			},
