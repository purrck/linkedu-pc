      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="messageTitle" label="消息" sortable="custom" width="400"></el-table-column>
            <el-table-column prop="sendDt" label="发送时间" sortable="custom" width="200"></el-table-column>
            <!--<el-table-column prop="isRead" label="已读" sortable="custom">-->
            ...
            <el-table-column label="消息类型">
              <template slot-scope="scope">
                <span v-if="scope.row.messageTypeCodeStr" :style="{color:scope.row.messageTypeCodeStr.split(';')[1] }">{{scope.row.messageTypeCodeStr.split(';')[0]}}</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right" width="300">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.messageId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.messageId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.messageId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    data() {
      return {
        ...
        ynOptions: [{'label': '全部', 'name': ''}], ynValue: null,
        messageTypeCodeOptions: [{'label': '全部', 'name': ''}],
        messageTypeCodeValue: null,
        form: {},
        searchForm: {}
        ,
        data: '',
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/safety/editSystemmessage/add/' + 0
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/editSystemmessage/edit/' + id
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/editSystemmessage/detail/' + id
        this.$router.push({path: opath})
      },
      getsendDtStr(row) {
        return row.sendDtStr
      },
      getisReadStr(row) {
        return row.isReadStr
      },
      getmessageTypeCodeStr(row) {
        return row.messageTypeCodeStr
      },
    }
  }
</script>
