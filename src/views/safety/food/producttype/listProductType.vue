      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="食品类型:" prop="productTypeId">
            <el-input class="input-v2-rao" v-model="searchForm.productTypeId" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="上级类型:" prop="productTypeParentName">
            <el-input class="input-v2-rao" v-model="searchForm.productTypeParentName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="description" align="center" label="描述" sortable="custom">
              <template slot-scope="scope">
                <div v-html="scope.row.description" class="one-line"></div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="productTypeParentId" align="center" label="上级类型" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.productTypeParentId" class="one-line">{{scope.row.productTypeParentId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="productTypeParentName" align="center" label="上级类型" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.productTypeParentName" class="one-line">{{scope.row.productTypeParentName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.productTypeId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.productTypeId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.productTypeId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                  </el-popover>

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="食材商品" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.productTypeId ,`listProductInfo`)"><i
...
<script>
  import api from "./producttypeUrl";
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/safety/detailProductType/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/ProductTypeTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailProductType/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailProductType/detail/' + id;
        this.$router.push({path: opath})
      },
      toAnother(id, path) {
        let opath = '/safety/' + path + '?productTypeId=' + id;
        this.$router.push({path: opath})
      },
    }
  }
