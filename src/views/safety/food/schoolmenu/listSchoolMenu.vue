    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="学校菜谱:" prop="menuId">
            <el-input class="input-v2-rao" v-model="searchForm.menuId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="菜谱类型:" prop="menuTypeCode">
            <el-select class="select-v2-rao" v-model="searchForm.menuTypeCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="menuTypeCode" label="菜谱类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.menuTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.menuTypeCodeStr">{{scope.row.menuTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="menuName" align="center" label="菜谱" sortable="custom">
              <template slot-scope="scope">
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.menuId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.menuId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.menuId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                  </el-popover>

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="学校菜谱食材" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.menuId ,`listSchoolMenuProduct`)"><i
                        class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>
                  </el-popover>

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="学校今日菜谱" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.menuId ,`listSchoolMenuDaily`)"><i
                        class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>
                  </el-popover>
                </div>
              </template>
  ...
  export default {
    data() {
      return {
        ...
        menuTypeCodeOptions: [{"label": "全部", "name": ""}],
        menuTypeCodeValue: null,
        form: {
          menuId: null,
          menuTypeCodeValue: null,
          menuName: null,
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/safety/detailSchoolMenu/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/SchoolMenuTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailSchoolMenu/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailSchoolMenu/detail/' + id;
        this.$router.push({path: opath})
      },
      toAnother(id, path) {
        let opath = '/safety/' + path + '?menuId=' + id;
        this.$router.push({path: opath})
      },
    }
  }
