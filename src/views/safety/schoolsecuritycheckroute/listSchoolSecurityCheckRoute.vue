<template>
  <div class="SchoolSecurityCheckRoute">
...
<!--          <el-form-item label="路线状态:" class="w140">-->
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="routeName" label="路线名称" sortable="custom"  align="center">
            ...
            <el-table-column prop="placeNum" label="巡检点数量" sortable="custom" align="center">
              <template slot-scope="scope">
                <div  class="one-line">{{scope.row.placeNum}}</div>
              </template>
            </el-table-column>
            <el-table-column prop="sum" label="巡检次数" sortable="custom" align="center">
              <template slot-scope="scope">
                <div class="one-line">{{scope.row.sum}}</div>
              </template>
            </el-table-column>
            <el-table-column prop="exceptionNum" label="异常次数" sortable="custom" align="center">
              <template slot-scope="scope">
                <div  class="one-line">{{scope.row.exceptionNum}}</div>
              </template>
            </el-table-column>
            <el-table-column prop="sequnceNum" label="级别" sortable="custom" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.sequnceNum" class="one-line">{{scope.row.sequnceNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="createdBy" label="创建人" sortable="custom" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.createdBy" class="one-line">{{scope.row.createdBy}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="createdDt" label="创建时间" sortable="custom" align="center">
              <template slot-scope="scope">
                <div v-if="scope.row.createdDt" class="one-line">{{scope.row.createdDt}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="sequnceNum" label="设置"  width="200" align="center">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="$router.push(`/safety/detailSchoolSecurityPlace/add/0?routeId=${scope.row.checkRouteId}`)">新增下级</el-button>
                <el-button size="mini" style="margin-left: 0" type="primary" @click="$router.push(`/safety/listSchoolSecurityPlace?routeId=${scope.row.checkRouteId}`)">查看下级</el-button>
              </template>
            </el-table-column>
            <el-table-column label="操作"  fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                  <span slot="reference" @click.stop="toEdit(scope.row.checkRouteId)"><i class="iconfont icon-icon_bianji ml5 f20 c999999"></i></span>
                </el-popover>

                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                  <span slot="reference" @click.stop="toDetail(scope.row.checkRouteId)"><i class="iconfont icon-icon_chakan1 ml5 f20 c999999"></i></span>
                </el-popover>
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference" @click.stop="rowDel(scope.row.checkRouteId)"><i class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>

                <!--<el-popover placement="top-start" popper-class="popover-v2-rao" title="巡检路线的巡检点" trigger="hover">-->
                  <!--<span slot="reference" @click.stop="toAnother(scope.row.checkRouteId ,`listSchoolSecurityCheckRouteItem`)"><i class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>-->
                <!--</el-popover>-->
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/safety/detailSchoolSecurityCheckRoute/add/' + 0
      this.$router.push({path: opath})
    },
    toEdit (id) {
      let opath = '/safety/detailSchoolSecurityCheckRoute/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/safety/detailSchoolSecurityCheckRoute/detail/' + id
      this.$router.push({path: opath})
    },
    toAnother (id, path) {
      let opath = '/safety/' + path +'?checkRouteId=' + id
      this.$router.push({path: opath})
    },
  }
}
