<template>
	<div class="SecurityOrganizationContact">
		<el-container v-loading="isLoaded" element-loading-custom-class="home-loading" style="min-height: 600px" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.1)">
			<el-header style="height: auto">
				...
				<el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
					<el-form-item label="安全联防机构:" prop="securityOrganizationId">
						<el-input class="input-v2-rao" v-model="searchForm.securityOrganizationId" maxlength="30"></el-input>
					</el-form-item>
					<el-form-item label="联系人:" prop="contactName">
						<el-input class="input-v2-rao" v-model="searchForm.contactName" maxlength="30"></el-input>
					</el-form-item>
			...
			<el-main class="pt-15">
				...
				<div class="table">
					<el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border @selection-change="handleSelectionChange" class="table-v2-rao" ref="schooltable" header-row-class-name="table-header-v2-rao" :stripe="true" @sort-change="sortChange">
						...
						<el-table-column prop="contactName" align="center" label="联系人" sortable="custom">
							<template slot-scope="scope">
								<div v-if="scope.row.contactName" class="one-line">{{scope.row.contactName}}</div>
								<div v-else>无</div>
							</template>
						</el-table-column>
							...
							<template slot-scope="scope">
								<div class="flex  row">
									...
									<el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
										<span slot="reference" @click.stop="toEdit(scope.row.organizationContactId)"><i
                        class="iconfont icon-icon_bianji ml5 f20 c999999"></i></span>
									...
									<el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
										<span slot="reference" @click.stop="toDetail(scope.row.organizationContactId)"><i
                        class="iconfont icon-icon_chakan1 ml5 f20 c999999"></i></span>
									...
									<el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
										<span slot="reference" @click.stop="rowDel(scope.row.organizationContactId)"><i
		...
		methods: {
			...
			handleSelectionChange(val) {
				this.multipleSelection = val.map(item => {
					return item.organizationContactId
				});
			},
...
//					// console.log(this.params)
			...
			toAdd() {
				...
				let opath = '/safety/detailSecurityOrganizationContact/add/' + this.params.securityOrganizationId;
				this.$router.push({
					path: opath
				})
			},
			toTotal() {
				let opath = '/safety/SecurityOrganizationContactTotal';
				this.$router.push({
					path: opath
				})
			},
			toEdit(id) {
				let opath = '/safety/detailSecurityOrganizationContact/edit/' + id;
				this.$router.push({
					path: opath
				})
			},
			toDetail(id) {
				let opath = '/safety/detailSecurityOrganizationContact/detail/' + id;
				this.$router.push({
					path: opath
				})
			},
