<template>
  <div class="StudentTimelineCheckDetail">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="考勤:" prop="timelineCheckId">
            <el-input class="input-v2-rao" v-model="searchForm.timelineCheckId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="作息名称:" prop="timelineDetailName">
            <el-input class="input-v2-rao" v-model="searchForm.timelineDetailName" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="考勤日期:" prop="calendarDt">
            <el-date-picker class="date-v2-rao" v-model="searchForm.calendarDt" type="date"
          ...
          <el-form-item label="正常考勤:" prop="isNormal">
            <el-select class="select-v2-rao" v-model="searchForm.isNormal" placeholder="请您选择">
              <el-option v-for="isNormalItem in ynOptions"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="timelineDetailName" align="center" label="作息名称" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.timelineDetailName" class="one-line">{{scope.row.timelineDetailName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="calendarDt" label="考勤日期" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="timelineTypeCode" label="作息类型" align="center" sortable="custom">
              <template slot-scope="scope">
            ...
            <el-table-column prop="checkedBeginTime" align="center" label="考勤时间" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.checkedBeginTime" class="one-line">{{scope.row.checkedBeginTime}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="checkedEndTime" align="center" label="考勤结束时间" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.checkedEndTime" class="one-line">{{scope.row.checkedEndTime}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="isNormal" label="正常考勤" align="center" sortable="custom" >
              <template slot-scope="scope">
                <div class="switch-el2">
                  <el-switch class="" v-model="scope.row.isNormal == 'Y'" disabled></el-switch>
                </div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <!--<el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">-->
                    <!--<span slot="reference"-->
                          <!--@click.stop="toEdit(scope.row.timelineCheckDetailId)"><i-->
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.timelineCheckDetailId)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.timelineCheckDetailId)"><i
...
<script>
  import api from "./studenttimelinecheckdetailUrl";
  ...
  export default {
    ...
    created() {
      let timelineCheckId = this.$route.query.timelineCheckId;
      this.params.timelineCheckId = timelineCheckId;
    },
    mounted() {
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        this.multipleSelection = val.map(item => {
          return item.timelineCheckDetailId
        });
      },
      ...
      toAdd() {
        ...
        let opath = '/safety/detailStudentTimelineCheckDetail/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/StudentTimelineCheckDetailTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailStudentTimelineCheckDetail/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailStudentTimelineCheckDetail/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
