    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          <el-form-item label="实体名(英):">
            <el-input v-model="searchForm.entityCnName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="实体名(中):">
            <el-input v-model="searchForm.entityEnName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="entityCnName" label="实体名(英)" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="entityEnName" label="实体名(中)" sortable="custom" width="200"></el-table-column>
            <el-table-column label="标识图" width="200">
            ...
            <el-table-column prop="isCanShare" label="可分享" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isCanShare == 'Y'" disabled></el-switch>
                </div>

              </template>
            </el-table-column>
            <el-table-column prop="isCanReply" label="可点评" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isCanReply == 'Y'" disabled></el-switch>
                </div>

              </template>
            </el-table-column>
            <el-table-column prop="isCanCollect" label="可收藏" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isCanCollect == 'Y'" disabled></el-switch>
                </div>

              </template>
            </el-table-column>
            <el-table-column prop="searchIndex" label="搜索索引" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="isHaveImages" label="图集" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isHaveImages == 'Y'" disabled></el-switch>
                </div>

              </template>
            </el-table-column>
            <el-table-column prop="tableName" label="对应表名" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="tablePrimary" label="主键字段" sortable="custom" width="200"></el-table-column>
            <el-table-column label="操作" width="300" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           class="tableEditButton"
                           @click.stop="toEdit(scope.row.entityTypeId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.entityTypeId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.entityTypeId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/safety/editEntitytype/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/editEntitytype/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/editEntitytype/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
