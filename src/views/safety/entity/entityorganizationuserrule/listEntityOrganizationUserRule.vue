      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="belongName" label="所属机构" sortable="custom" width="200"></el-table-column>
            ...
            <el-table-column prop="organizedUserNum" label="组织用户数" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="participatedUserNum" label="参与用户数" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="completedUserNum" label="完成用户数" sortable="custom" width="200"></el-table-column>
            <el-table-column label="状态">
              <template slot-scope="scope">
                <!--<span :class="scope.row.entityOrgRuleStatusCode"-->
                <!--:style="{color:scope.row.entityOrgRuleStatusCodeStr.split(';')[1]}">{{scope.row.entityOrgRuleStatusCode.split(';')[0]}}</span>-->
              </template>
            </el-table-column>
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.entityOrgRuleId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.entityOrgRuleId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.entityOrgRuleId)">删除
                </el-button>
                <!--<el-button size="small" type="primary"  @click="toDetail(scope.row.entityOrgRuleId)">-->
                <!--安全教育组织用户-->
                <!--</el-button>-->
              </template>
            </el-table-column>
  ...
  export default {
    data() {
      return {
        ...
        entityOrgRuleStatusCodeOptions: [{"label": "全部", "name": ""}],
    ...
    created() {
      let entityOrgId = this.$route.query.entityOrgId;
      this.params.entityOrgId = entityOrgId;
    },
    mounted() {
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/safety/detailEntityOrganizationUserRule/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/detailEntityOrganizationUserRule/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/detailEntityOrganizationUserRule/detail/' + id;
        this.$router.push({path: opath})
      },
      ...
      getentityOrgRuleStatusCodeStr(row) {
        return row.entityOrgRuleStatusCodeStr;
      },
    }
  }
</script>
