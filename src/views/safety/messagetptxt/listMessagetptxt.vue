      ...
      <el-header style="height: auto">
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          <el-form-item label="内容标题:">
            <el-input v-model="searchForm.textLabel" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="类型（头 底）:">
            <!--<el-input v-model="searchForm.textType" maxlength="30"></el-input>-->
            <el-select v-model="searchForm.textType">
              <el-option v-for="item in TypeCodeOptions" :label="item.label" :value="item.name"
                         :key="item.name"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="内容:">
            <el-input v-model="searchForm.textContent" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="textLabel" label="内容标题" sortable="custom"></el-table-column>
            <el-table-column prop="textType" label="类型（头 底）" sortable="custom" width="200">
              <template slot-scope="scope">
                <div v-if="scope.row.textType==='H'">头部</div>
                <div v-else>底部</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right" width="300">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.textId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.textId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.textId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/safety/editMessagetptxt/add/' + 0
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/safety/editMessagetptxt/edit/' + id
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/safety/editMessagetptxt/detail/' + id
        this.$router.push({path: opath})
      },
    }
  }
