      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="lostThingReportDt" label="上报时间" align="center" sortable="custom"></el-table-column>
            <!-- 上报时间 colName:lost_thing_report_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="pickupUserTypeCode" label="拾取人类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.pickupUserTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.pickupUserTypeCodeStr">{{scope.row.pickupUserTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="pickupDt" label="拾取时间" align="center" sortable="custom"></el-table-column>
            ...
            <el-table-column prop="applyedUserType" align="center" label="认领人类型" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.applyedUserType" class="one-line">{{scope.row.applyedUserType}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="applyedDt" label="认领时间" align="center" sortable="custom"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/securityReminder/detailLostThing/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/securityReminder/LostThingTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/securityReminder/detailLostThing/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/securityReminder/detailLostThing/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
