    ...
    >
      <el-header style="height: auto">
        <el-form
          ...
          class="select-form"
        >
          <el-form-item label="校内任务:" prop="taskId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.taskId" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="需要上传文件:" prop="isNeedFile" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.isNeedFile" placeholder="请您选择">
              <el-option
                v-for="isNeedFileItem in ynOptions"
                :key="isNeedFileItem.name"
                :label="isNeedFileItem.label"
                :value="isNeedFileItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="taskId" align="center" label="校内任务" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.taskId" class="one-line">{{scope.row.taskId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 校内任务 colName:task_id sqlType:VARCHAR notnull:false -->
            ...
            <el-table-column prop="taskSendDt" label="发起时间" align="center" sortable="custom"></el-table-column>
            <!-- 发起时间 colName:task_send_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                >
                  <span
                    slot="reference"
                    @click.stop="toAnother(scope.row.taskId ,`listSchoolTaskUser`)"
                  >
...
export default {
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/securityReminder/detailSchoolTask/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/securityReminder/SchoolTaskTotal";
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/securityReminder/detailSchoolTask/detail/" + id;
      this.$router.push({ path: opath });
    },
    toAnother(id, path) {
      let opath = "/securityReminder/" + path + "?taskId=" + id;
      this.$router.push({ path: opath });
    }
  }
};
