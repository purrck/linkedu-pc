    ...
    >
      <el-header style="height: auto">
        <el-form
          ...
          class="select-form"
        >
          <el-form-item label="小组:" prop="groupId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.groupId" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="职责:" prop="groupUserDutyCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.groupUserDutyCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="groupUserDutyCodeItem in groupUserDutyCodeOptions"
                :key="groupUserDutyCodeItem.name"
                :label="groupUserDutyCodeItem.label"
                :value="groupUserDutyCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="备注:" prop="remark" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.remark" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="加入时间:" prop="joinedDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.joinedDt"
              type="date"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="groupUserDutyCode" label="职责" align="center" sortable="custom">
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.groupUserDutyCodeStr.split(';')[1]}"
                  v-if="scope.row.groupUserDutyCodeStr"
                >{{scope.row.groupUserDutyCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="joinedDt" label="加入时间" align="center" sortable="custom"></el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.groupUserId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.groupUserId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.groupUserId)">
...
export default {
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("groupUserDuty").then(
      res => {
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/securityReminder/detailStudentGroupUser/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/securityReminder/StudentGroupUserTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath = "/securityReminder/detailStudentGroupUser/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/securityReminder/detailStudentGroupUser/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
