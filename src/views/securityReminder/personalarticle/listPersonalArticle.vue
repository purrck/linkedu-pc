...
<!-- 发布人 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="isPersonalRecommend" label="个人置顶" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isPersonalRecommend == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
...
<!-- 序号 colName:sequnce_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="readTimesNum" align="center" label="阅读" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.readTimesNum" class="one-line">{{scope.row.readTimesNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 阅读 colName:read_times_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="praiseTimesNum" align="center" label="点赞" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.praiseTimesNum" class="one-line">{{scope.row.praiseTimesNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
...
<!-- 踩 colName:dislike_times_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="commentTimesNum" align="center" label="评论" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.commentTimesNum" class="one-line">{{scope.row.commentTimesNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/securityReminder/detailPersonalArticle/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/securityReminder/PersonalArticleTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/securityReminder/detailPersonalArticle/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/securityReminder/detailPersonalArticle/detail/' + id;
        this.$router.push({path: opath})
      },
                  toAnother(id, path) {
        let opath = '/securityReminder/' + path + '?articleId=' + id;
        this.$router.push({path: opath})
      },
          }
  }
