    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="回答内容:"  prop="answerContent" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.answerContent" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="赞成:"  prop="approvedNum" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.approvedNum" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="反对:"  prop="disapprovedNum" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.disapprovedNum" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="被采纳:"  prop="isAccept" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isAccept" placeholder="请您选择">
                    <el-option v-for="isAcceptItem in ynOptions"
                               :key="isAcceptItem.name"
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="answerContent" align="center" label="回答内容" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.answerContent" class="one-line">{{scope.row.answerContent}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
...
<!-- 附件 colName:file_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="userName" align="center" label="回答人" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 回答人 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="answerDt" label="回答时间" align="center" sortable="custom"></el-table-column>
<!-- 回答时间 colName:answer_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="approvedNum" align="center" label="赞成" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.approvedNum" class="one-line">{{scope.row.approvedNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 赞成 colName:approved_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="disapprovedNum" align="center" label="反对" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.disapprovedNum" class="one-line">{{scope.row.disapprovedNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 反对 colName:disapproved_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="isAccept" label="被采纳" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isAccept == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
<!-- 被采纳 colName:is_accept sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.answerId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.answerId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.answerId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/questionAnswer/detailQuestionAnswer/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/questionAnswer/QuestionAnswerTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/questionAnswer/detailQuestionAnswer/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/questionAnswer/detailQuestionAnswer/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
