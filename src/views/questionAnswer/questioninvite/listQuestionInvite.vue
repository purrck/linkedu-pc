        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="questionTitle" align="center" label="问题标题" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.questionTitle" class="one-line">{{scope.row.questionTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 问题标题 colName:question_title sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="inviteContent" align="center" label="邀请" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.inviteContent" class="one-line">{{scope.row.inviteContent}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 邀请 colName:invite_content sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="userName" align="center" label="被邀请人" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 被邀请人 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="invitedDt" label="邀请时间" align="center" sortable="custom"></el-table-column>
<!-- 邀请时间 colName:invited_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.inviteId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.inviteId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.inviteId)"><i
...
<script>
  import api from "./questioninviteUrl";
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/questionAnswer/detailQuestionInvite/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/questionAnswer/QuestionInviteTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/questionAnswer/detailQuestionInvite/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/questionAnswer/detailQuestionInvite/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
