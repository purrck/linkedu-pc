    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="标签:"  prop="tag" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.tag" maxlength="30"></el-input>
                </el-form-item>
          <el-form-item>
...
<!-- 问题类型 colName:question_type_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="tag" align="center" label="标签" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.tag" class="one-line">{{scope.row.tag}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 标签 colName:tag sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.questionTagId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.questionTagId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.questionTagId)"><i
...
<script>
  import api from "./questiontagUrl";
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/questionAnswer/detailQuestionTag/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/questionAnswer/QuestionTagTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/questionAnswer/detailQuestionTag/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/questionAnswer/detailQuestionTag/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
