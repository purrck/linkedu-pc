    ...
    >
      <el-header style="height: auto">
        ...
        >
          ...
          <el-form-item label="请假:" prop="leaveTitle" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.leaveTitle" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="请假类型:" prop="teacherLeaveTypeCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.teacherLeaveTypeCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="teacherLeaveTypeCodeItem in teacherLeaveTypeCodeOptions"
                :key="teacherLeaveTypeCodeItem.name"
                :label="teacherLeaveTypeCodeItem.label"
                :value="teacherLeaveTypeCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="开始时间:" prop="leaveBeginDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.leaveBeginDt"
              type="date"
              value-format="yyyy-MM-dd HH:mm"
              placeholder="选择日期"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="截止时间:" prop="leaveEndDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.leaveEndDt"
              type="date"
              value-format="yyyy-MM-dd HH:mm"
              placeholder="选择日期"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="请假时长:" prop="leaveDays" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.leaveDays" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="请假状态:" prop="teacherLeaveStatusCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.teacherLeaveStatusCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="teacherLeaveStatusCodeItem in teacherLeaveStatusCodeOptions"
                :key="teacherLeaveStatusCodeItem.name"
                :label="teacherLeaveStatusCodeItem.label"
                :value="teacherLeaveStatusCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="leaveTitle" align="center" label="请假" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.leaveTitle" class="one-line">{{scope.row.leaveTitle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 请假 colName:leave_title sqlType:VARCHAR notnull:false -->
            <el-table-column
              prop="teacherLeaveTypeCode"
              label="请假类型"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.teacherLeaveTypeCodeStr.split(';')[1]}"
                  v-if="scope.row.teacherLeaveTypeCodeStr"
                >{{scope.row.teacherLeaveTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="leaveBeginDt" label="开始时间" align="center" sortable="custom"></el-table-column>
            <!-- 开始时间 colName:leave_begin_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="leaveEndDt" label="截止时间" align="center" sortable="custom"></el-table-column>
            <!-- 截止时间 colName:leave_end_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="leaveDays" align="center" label="请假时长" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.leaveDays" class="one-line">{{scope.row.leaveDays}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 请假时长 colName:leave_days sqlType:FLOAT notnull:false -->
            <el-table-column
              prop="teacherLeaveStatusCode"
              label="请假状态"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.teacherLeaveStatusCodeStr.split(';')[1]}"
                  v-if="scope.row.teacherLeaveStatusCodeStr"
                >{{scope.row.teacherLeaveStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <!-- 请假状态 colName:teacher_leave_status_code sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.teacherLeaveId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.teacherLeaveId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.teacherLeaveId)">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        teacherName: null,
        leaveTitle: null,
        teacherLeaveTypeCodeValue: null,
        leaveBeginDt: null,
        leaveEndDt: null,
        leaveDays: null,
        teacherLeaveStatusCodeValue: null
      },
      searchForm: {
        teacherName: null,
        leaveTitle: null,
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("teacherLeaveType").then(
      res => {
    ...
    commonApi.getCodeIntType("teacherLeaveStatus").then(
      res => {
        let viewList = res.data.data || [];
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/safetyManagement/detailSchoolTeacherLeave/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolTeacherLeaveTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath = "/safetyManagement/detailSchoolTeacherLeave/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/safetyManagement/detailSchoolTeacherLeave/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
