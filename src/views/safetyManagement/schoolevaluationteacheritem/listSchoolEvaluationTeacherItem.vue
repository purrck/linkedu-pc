...
<template>
  <div class="SchoolEvaluationTeacherItem">
    <el-container
      v-loading="isLoaded"
    ...
    >
      <el-header style="height: auto">
        ...
        >
          ...
          <el-form-item label="评教状态:" prop="evaluationTeacherItemStatusCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.evaluationTeacherItemStatusCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="evaluationTeacherItemStatusCodeItem in evaluationTeacherItemStatusCodeOptions"
                :key="evaluationTeacherItemStatusCodeItem.name"
                :label="evaluationTeacherItemStatusCodeItem.label"
                :value="evaluationTeacherItemStatusCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="评分:" prop="evaluateScore" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.evaluateScore" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column
              prop="evaluationTeacherItemStatusCode"
              label="评教状态"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.evaluationTeacherItemStatusCodeStr.split(';')[1]}"
                  v-if="scope.row.evaluationTeacherItemStatusCodeStr"
                >{{scope.row.evaluationTeacherItemStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="evaluateDt" label="评教时间" align="center" sortable="custom"></el-table-column>
            <!-- 评教时间 colName:evaluate_dt sqlType:TIMESTAMP notnull:false -->
            ...
            <el-table-column prop="answerId" align="center" label="问卷答案" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.answerId" class="one-line">{{scope.row.answerId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 问卷答案 colName:answer_id sqlType:VARCHAR notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.evaluationTeacherItemId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.evaluationTeacherItemId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.evaluationTeacherItemId)">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        teacherName: null,
        studentName: null,
        evaluationTeacherItemStatusCodeValue: null,
        evaluateScore: null
      },
      searchForm: {
        planId: null,
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("evaluationTeacherItemStatus").then(
      res => {
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      this.multipleSelection = val.map(item => {
        return item.evaluationTeacherItemId;
      });
    },
    ...
    toAdd() {
      ...
      let opath =
        "/safetyManagement/detailSchoolEvaluationTeacherItem/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolEvaluationTeacherItemTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath =
        "/safetyManagement/detailSchoolEvaluationTeacherItem/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath =
        "/safetyManagement/detailSchoolEvaluationTeacherItem/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
