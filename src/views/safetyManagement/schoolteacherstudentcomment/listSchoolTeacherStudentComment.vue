...
<template>
  <div class="SchoolTeacherStudentComment">
    <el-container
      v-loading="isLoaded"
    ...
    >
      <el-header style="height: auto">
        ...
        >
          ...
          <el-form-item label="学生教师关系:" prop="teacherStudentRelationCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.teacherStudentRelationCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="teacherStudentRelationCodeItem in teacherStudentRelationCodeOptions"
                :key="teacherStudentRelationCodeItem.name"
                :label="teacherStudentRelationCodeItem.label"
                :value="teacherStudentRelationCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="评级时间:" prop="commentDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.commentDt"
              type="date"
          ...
          <el-form-item label="评级级别:" prop="commentLevelCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.commentLevelCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="commentLevelCodeItem in commentLevelCodeOptions"
                :key="commentLevelCodeItem.name"
                :label="commentLevelCodeItem.label"
                :value="commentLevelCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column
              prop="teacherStudentRelationCode"
              label="学生教师关系"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.teacherStudentRelationCodeStr.split(';')[1]}"
                  v-if="scope.row.teacherStudentRelationCodeStr"
                >{{scope.row.teacherStudentRelationCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="commentDt" label="评级时间" align="center" sortable="custom"></el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.commentId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.commentId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.commentId)">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        studentName: null,
        teacherStudentRelationCodeValue: null,
        commentContent: null,
        commentDt: null,
        schoolYear: null,
        schoolTerm: null,
        commentLevelCodeValue: null
      },
      searchForm: {
        studentName: null,
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("teacherStudentRelation").then(
      res => {
    ...
    commonApi.getCodeIntType("commentLevel").then(
      res => {
        let viewList = res.data.data || [];
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath =
        "/safetyManagement/detailSchoolTeacherStudentComment/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolTeacherStudentCommentTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherStudentComment/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherStudentComment/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
