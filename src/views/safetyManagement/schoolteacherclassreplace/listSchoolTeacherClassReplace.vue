...
<template>
  <div class="SchoolTeacherClassReplace">
    <el-container
      v-loading="isLoaded"
    ...
    >
      <el-header style="height: auto">
        <el-form
          ...
          class="select-form"
        >
          <el-form-item label="代课老师:" prop="teacherName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.teacherName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="被代课老师:" prop="replaceTeacherName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.replaceTeacherName" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="代课状态:" prop="classReplaceStatusCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.classReplaceStatusCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="classReplaceStatusCodeItem in classReplaceStatusCodeOptions"
                :key="classReplaceStatusCodeItem.name"
                :label="classReplaceStatusCodeItem.label"
                :value="classReplaceStatusCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="teacherName" align="center" label="代课老师" sortable="custom">
            ...
            >
              <template slot-scope="scope">
                <div
                  v-if="scope.row.replaceTeacherName"
                  class="one-line"
                >{{scope.row.replaceTeacherName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="classBeginDt" label="上课时间" align="center" sortable="custom"></el-table-column>
            <!-- 上课时间 colName:class_begin_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="classEndDt" label="下课时间" align="center" sortable="custom"></el-table-column>
            <!-- 下课时间 colName:class_end_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column
              prop="classReplaceStatusCode"
              label="代课状态"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.classReplaceStatusCodeStr.split(';')[1]}"
                  v-if="scope.row.classReplaceStatusCodeStr"
                >{{scope.row.classReplaceStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <!-- 代课状态 colName:class_replace_status_code sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.classReplaceId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.classReplaceId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.classReplaceId)">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        teacherName: null,
        replaceTeacherName: null,
        className: null,
        gradeName: null,
        subjectName: null,
        classReplaceStatusCodeValue: null
      },
      searchForm: {
        teacherName: null,
        replaceTeacherName: null,
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("classReplaceStatus").then(
      res => {
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/safetyManagement/detailSchoolTeacherClassReplace/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolTeacherClassReplaceTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherClassReplace/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherClassReplace/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
