...
<template>
  <div class="SchoolTeacherPositionalTitle">
    <el-container
      v-loading="isLoaded"
    ...
    >
      <el-header style="height: auto">
        <el-form
          ...
          class="select-form"
        >
          <el-form-item label="职称级别:" prop="positionalTitleLevelCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.positionalTitleLevelCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="positionalTitleLevelCodeItem in positionalTitleLevelCodeOptions"
                :key="positionalTitleLevelCodeItem.name"
                :label="positionalTitleLevelCodeItem.label"
                :value="positionalTitleLevelCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="获得时间:" prop="gotDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.gotDt"
              type="date"
              value-format="yyyy-MM-dd HH:mm"
              placeholder="选择日期"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="获得地点:" prop="gotAddress" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.gotAddress" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="是否聘任:" prop="isAppointPosition" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.isAppointPosition"
              placeholder="请您选择"
            >
              <el-option
                v-for="isAppointPositionItem in ynOptions"
                :key="isAppointPositionItem.name"
                :label="isAppointPositionItem.label"
                :value="isAppointPositionItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="职称状态:" prop="teacherPositionalTitleStatusCode" class="w140">
            <el-select
              class="select-v2-rao"
              v-model="searchForm.teacherPositionalTitleStatusCode"
              placeholder="请您选择"
            >
              <el-option
                v-for="teacherPositionalTitleStatusCodeItem in teacherPositionalTitleStatusCodeOptions"
                :key="teacherPositionalTitleStatusCodeItem.name"
                :label="teacherPositionalTitleStatusCodeItem.label"
                :value="teacherPositionalTitleStatusCodeItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.positionalTitleLevelCodeStr.split(';')[1]}"
                  v-if="scope.row.positionalTitleLevelCodeStr"
                >{{scope.row.positionalTitleLevelCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <!-- 职称级别 colName:positional_title_level_code sqlType:VARCHAR notnull:false -->
            <el-table-column prop="gotDt" label="获得时间" align="center" sortable="custom"></el-table-column>
            <!-- 获得时间 colName:got_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="gotAddress" align="center" label="获得地点" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.gotAddress" class="one-line">{{scope.row.gotAddress}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 获得地点 colName:got_address sqlType:VARCHAR notnull:false -->
            <el-table-column prop="isAppointPosition" label="是否聘任" align="center" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isAppointPosition == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
            <!-- 是否聘任 colName:is_appoint_position sqlType:VARCHAR notnull:false -->
            <el-table-column
              prop="teacherPositionalTitleStatusCode"
              label="职称状态"
              align="center"
              sortable="custom"
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.teacherPositionalTitleStatusCodeStr.split(';')[1]}"
                  v-if="scope.row.teacherPositionalTitleStatusCodeStr"
                >{{scope.row.teacherPositionalTitleStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <!-- 职称状态 colName:teacher_positional_title_status_code sqlType:VARCHAR notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.positionalTitleId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.positionalTitleId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.positionalTitleId)">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        positionalTitleLevelCodeValue: null,
        gotDt: null,
        gotAddress: null,
        ynValue: null,
        teacherPositionalTitleStatusCodeValue: null,
        teacherName: null
      },
      searchForm: {
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("positionalTitleLevel").then(
      res => {
    ...
    commonApi.getCodeIntType("teacherPositionalTitleStatus").then(
      res => {
        let viewList = res.data.data || [];
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath =
        "/safetyManagement/detailSchoolTeacherPositionalTitle/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolTeacherPositionalTitleTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherPositionalTitle/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath =
        "/safetyManagement/detailSchoolTeacherPositionalTitle/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
