        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="studyGroupName" align="center" label="课堂教学分组" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.studyGroupName" class="one-line">{{scope.row.studyGroupName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
...
<!-- 年级 colName:grade_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="segementName" align="center" label="学段" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.segementName" class="one-line">{{scope.row.segementName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 学段 colName:segement_name sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.studyGroupId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.studyGroupId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.studyGroupId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safetyManagement/detailClassStudyGroup/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safetyManagement/ClassStudyGroupTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safetyManagement/detailClassStudyGroup/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safetyManagement/detailClassStudyGroup/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
