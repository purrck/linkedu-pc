    ...
    >
      <el-header style="height: auto">
        ...
        >
          ...
          <el-form-item label="值班日期:" prop="calendarDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
          ...
          <el-form-item label="签到时间:" prop="checkInDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.checkInDt"
              type="date"
              value-format="yyyy-MM-dd HH:mm"
              placeholder="选择日期"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="签离时间:" prop="checkOutDt" class="w140">
            <el-date-picker
              class="date-v2-rao"
              v-model="searchForm.checkOutDt"
              type="date"
              value-format="yyyy-MM-dd HH:mm"
              placeholder="选择日期"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="是否缺勤:" prop="isAbsenteeism" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.isAbsenteeism" placeholder="请您选择">
              <el-option
                v-for="isAbsenteeismItem in ynOptions"
                :key="isAbsenteeismItem.name"
                :label="isAbsenteeismItem.label"
                :value="isAbsenteeismItem.name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="calendarDt" label="值班日期" align="center" sortable="custom"></el-table-column>
            <!-- 值班日期 colName:Calendar_DT sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="checkInDt" label="签到时间" align="center" sortable="custom"></el-table-column>
            <!-- 签到时间 colName:check_in_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="checkOutDt" label="签离时间" align="center" sortable="custom"></el-table-column>
            <!-- 签离时间 colName:check_out_dt sqlType:TIMESTAMP notnull:false -->
            <el-table-column prop="isAbsenteeism" label="是否缺勤" align="center" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isAbsenteeism == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
            <!-- 是否缺勤 colName:is_absenteeism sqlType:VARCHAR notnull:false -->
            <el-table-column prop="recordContent" align="center" label="值班日志" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.recordContent" class="one-line">{{scope.row.recordContent}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 值班日志 colName:record_content sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.beondutyCheckId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.beondutyCheckId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.beondutyCheckId)">
...
export default {
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/safetyManagement/detailSchoolBeondutyCheck/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/safetyManagement/SchoolBeondutyCheckTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath = "/safetyManagement/detailSchoolBeondutyCheck/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/safetyManagement/detailSchoolBeondutyCheck/detail/" + id;
      this.$router.push({ path: opath });
    }
  }
};
