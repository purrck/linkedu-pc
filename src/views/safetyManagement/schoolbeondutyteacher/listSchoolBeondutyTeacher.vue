...
<template>
  <div class="SchoolBeondutyTeacher">
...
<!-- 教师 colName:teacher_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="markScore" align="center" label="值班评分" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.markScore" class="one-line">{{scope.row.markScore}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.beondutyTeacherId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.beondutyTeacherId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.beondutyTeacherId)"><i
...
<script>
  import api from "./schoolbeondutyteacherUrl";
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safetyManagement/detailSchoolBeondutyTeacher/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safetyManagement/SchoolBeondutyTeacherTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safetyManagement/detailSchoolBeondutyTeacher/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safetyManagement/detailSchoolBeondutyTeacher/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
