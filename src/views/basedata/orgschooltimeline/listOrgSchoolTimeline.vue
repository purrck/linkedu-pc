            ...
            <el-table-column label="操作" fixed="right" width="180">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="学校日常作息安排" trigger="hover">
                      <span slot="reference"
                            @click.stop="toAnother(scope.row.timelineId ,`listOrgSchoolTimelineDetailXs`)"><i
...
export default {
  ...
  methods: {
    ...
    toAdd () {
      ...
      let opath = '/basedata/detailOrgSchoolTimelineXs/add/' + 0
    ...
    toEdit (id) {
      let opath = '/basedata/detailOrgSchoolTimelineXs/edit/' + id
      this.$router.push({path: opath})
    },
    toDetail (id) {
      let opath = '/basedata/detailOrgSchoolTimelineXs/detail/' + id
      this.$router.push({path: opath})
    },
    toAnother (id, path) {
      let opath = '/basedata/' + path + '?timelineId=' + id
      this.$router.push({path: opath})
    },
  }
}
