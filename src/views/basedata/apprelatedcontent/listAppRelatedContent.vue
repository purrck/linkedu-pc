      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="appName" label="应用" sortable="custom" width="200">
            ...
            <el-table-column label="文案类型">
              <template slot-scope="scope">
                <span  v-if="scope.row.contentTypeCodeStr" :style="{color:scope.row.contentTypeCodeStr.split(';')[1]}" >
                      {{scope.row.contentTypeCodeStr.split(';')[0]}}</span>
                <span  v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column label="文案状态">
              <template slot-scope="scope">
                <span :style="{color:scope.row.contentStatusCodeStr.split(';')[1]}"
                      v-if="scope.row.contentStatusCodeStr">{{scope.row.contentStatusCodeStr.split(';')[0]}}</span>
                <span  v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="contentTitle" label="文案" sortable="custom" width="200">
              <template slot-scope="scope">
                <div v-if="scope.row.contentTitle">{{scope.row.contentTitle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="350" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.contentId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.contentId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.contentId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    data() {
      return {
        ...
        contentTypeCodeOptions: [{"label": "全部", "name": ""}],
        contentTypeCodeValue: null,
        contentStatusCodeOptions: [{"label": "全部", "name": ""}],
        contentStatusCodeValue: null,
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/basedata/detailAppRelatedContent/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailAppRelatedContent/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailAppRelatedContent/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
