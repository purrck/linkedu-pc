      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="complaintNums" label="投诉编号" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="complaintUserName" label="投诉客户名称" sortable="custom" width="200"></el-table-column>
            <el-table-column label="处理状态">
              <template slot-scope="scope">
                <span :class="scope.row.replyStatusCode" :style="{color:sscope.row.replyStatusCodeStr.split(';')[1]}">{{scope.row.replyStatusCode.split(';')[0]}}</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.replyId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
  ...
  export default {
    data() {
      return {
        ...
        replyStatusCodeOptions: [{"label": "全部", "name": ""}],
        replyStatusCodeValue: null,
        form: {
          complaintId:
            null,
        },
    ...
    created() {
      let complaintId = this.$route.query.complaintId;
      this.params.complaintId = complaintId;
    },
    activated() {
      //只刷新数据，不改变整体的缓存
      // console.log('重新活跃路由');
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/basedata/detailComplaintReply/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailComplaintReply/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailComplaintReply/detail/' + id;
        this.$router.push({path: opath})
      },
      getreplyStatusCodeStr(row) {
        return row.replyStatusCodeStr;
      },
    }
  }
</script>
