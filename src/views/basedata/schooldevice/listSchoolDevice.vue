    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="设备类型:" prop="schoolDeviceTypeCode" class="w140">
          ...
          <el-form-item label="设备号:" prop="deviceNum" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.deviceNum" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="设备状态:" prop="schoolDeviceStatusCode" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.schoolDeviceStatusCode" placeholder="请您选择">
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="schoolDeviceTypeCode" label="设备类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.schoolDeviceTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.schoolDeviceTypeCodeStr">{{scope.row.schoolDeviceTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="deviceGroup" align="center" label="设备分组" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.deviceGroup" class="one-line">{{scope.row.deviceGroup}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="inOutStateCode" align="center" label="内/外标识" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.inOutStateCode==='in'" class="one-line">进入</div>
                <div v-if="scope.row.inOutStateCode==='out'" class="one-line">外出</div>
                <div v-if="scope.row.inOutStateCode===''">无</div>
              </template>
            </el-table-column>


            <el-table-column prop="schoolDeviceStatusCode" label="设备状态" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.schoolDeviceStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.schoolDeviceStatusCodeStr">{{scope.row.schoolDeviceStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.schoolDeviceId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.schoolDeviceId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.schoolDeviceId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form: {
          schoolDeviceTypeCodeValue: null,
          deviceNum: null,
          placeName: null,
          schoolDeviceStatusCodeValue: null,
        },
        searchForm: {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/basedata/detailSchoolDevice/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/basedata/SchoolDeviceTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailSchoolDevice/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailSchoolDevice/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
