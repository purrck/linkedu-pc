            ...
            <el-table-column label="操作" width="300" fixed="right">
              ...
              <template slot-scope="scope">
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                  <span slot="reference"
                        @click.stop="toEdit(scope.row.entityOrgRuleId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.appId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.appId)"><i
                    class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="应用安装用户" trigger="hover">
                  <span slot="reference"
                        @click.stop="toAnother(scope.row.appId ,`listAppUsers`)"><i
                    class="iconfont icon-icon_yingyonganzhuangyonghu ml5 f20 c999999"></i></span>
                </el-popover>
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="应用关联文案" trigger="hover">
                  <span slot="reference"
                        @click.stop="toAnother(scope.row.appId ,`listAppRelatedContent`)"><i
                    class="iconfont icon-icon_yingyongguanlianwenan ml5 f20 c999999"></i></span>
                </el-popover>
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="应用版本" trigger="hover">
                  <span slot="reference"
                        @click.stop="toAnother(scope.row.appId ,`listAppVersions`)"><i
                    class="iconfont icon-icon_yingyongbanben ml5 f20 c999999"></i></span>
                </el-popover>
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="应用欢迎图集" trigger="hover">
                  <span slot="reference"
                        @click.stop="toAnother(scope.row.appId ,`listAppWelcomeTheme`)"><i
  ...
  export default {
    data() {
      return {
        ...
        apptaskStatusCodeOptions: [{"label": "全部", "name": ""}],
        apptaskStatusCodeValue: null,
        form: {
          appId: null,
          appName: null,
        },
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/basedata/detailAppList/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailAppList/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailAppList/detail/' + id;
        this.$router.push({path: opath})
      },
      toAnother(id, path) {
        let opath = '/basedata/' + path + '?appId=' + id;
        this.$router.push({path: opath})
      },
    }
  }
