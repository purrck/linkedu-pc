    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          ...
          <el-form-item label="设备编码:" prop="deviceNum" class="w140">
            <!--（rfid唯一标识，手环编码等）-->
            <el-input class="input-v2-rao" v-model="searchForm.deviceNum" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="设备类型:" prop="studentDeviceTypeCode" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.studentDeviceTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="是否禁止使用:" prop="isDisabled" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.isDisabled" placeholder="请您选择">
              <el-option v-for="isDisabledItem in ynOptions"
                         :key="isDisabledItem.name"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="deviceNum" align="center" label="设备编码" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.deviceNum" class="one-line">{{scope.row.deviceNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="studentDeviceTypeCode" label="设备类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.studentDeviceTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.studentDeviceTypeCodeStr">{{scope.row.studentDeviceTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="isDisabled" label="是否禁止使用" align="center" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isDisabled == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.studentDeviceId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.studentDeviceId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.studentDeviceId)"><i
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        form: {
          studentName: null,
          deviceNum: null,
          studentDeviceTypeCodeValue: null,
          ynValue: null,
        },
        searchForm: {
          studentName: null,
    ...
    methods: {
      ...
      toAdd() {
        //safety safetyEdu basedata    '?studentId=' + id+'&userName='+name+'&userId='+userId;
        let opath = '/basedata/detailStudentDevice/add/' + this.$route.query.studentId + '?userName=' + this.$route.query.userName + '&userId=' + this.$route.query.userId;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/basedata/StudentDeviceTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailStudentDevice/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailStudentDevice/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
