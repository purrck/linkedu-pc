      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          ...
          <el-form-item label="提交时间:">
            <el-date-picker class="date-v2-rao" v-model="searchForm.commitedDt" type="date"
                            value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
          </el-form-item>
          <el-form-item label="服务地址:">
            <el-input class="input-v2-rao" v-model="searchForm.serverAddress" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="远程地址:">
            <el-input class="input-v2-rao" v-model="searchForm.remoteAddress" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label=":">
            <el-input class="input-v2-rao" v-model="searchForm.uri" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="content" label=" 内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.content" class="one-line">{{scope.row.content}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="commitedDt" label="提交时间" sortable="custom"></el-table-column>
            <el-table-column prop="serverAddress" label="服务地址" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.serverAddress" class="one-line">{{scope.row.serverAddress}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="remoteAddress" label="远程地址" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.remoteAddress" class="one-line">{{scope.row.remoteAddress}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="uri" label="" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.uri" class="one-line">{{scope.row.uri}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column label="操作"  fixed="right" width="100">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                      <span slot="reference"
                            @click.stop="toDetail(scope.row.hisId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.hisId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/basedata/detailSensitiveWordsHis/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        //safety safetyEdu basedata
        let opath = '/basedata/sensitiveHisTotal'
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailSensitiveWordsHis/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
