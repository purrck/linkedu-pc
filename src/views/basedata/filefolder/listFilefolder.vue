      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="folderName" label="文件夹名" sortable="custom" width="250"></el-table-column>
            <el-table-column label="图标" width="250">
              <template slot-scope="scope">
                <div v-if="scope.row.imagePath" class="flex center">
                  <img :src="scope.row.imagePath" alt="" width="84px" class="scope-img">
                </div>
                <div v-if="!scope.row.imagePath">无</div>
              </template>
            </el-table-column>
            <el-table-column prop="description" label="描述" sortable="custom" width="250">
              <template slot-scope="scope">
                <div v-if="scope.row.description">
                  <span>{{scope.row.description}}</span>
                </div>
                <div v-if="!scope.row.description">无</div>
              </template>
            </el-table-column>
            <el-table-column prop="filesNum" label="文件个数" sortable="custom" width="200"></el-table-column>
            <el-table-column prop="isSelf" label="私有" sortable="custom">
              <template slot-scope="scope">
                <div>
                  <el-switch v-model="scope.row.isSelf == 'Y'" disabled></el-switch>
                </div>
            ...
            <el-table-column label="操作" width="400">
              <template slot-scope="scope">
                <el-button size="small" type="primary" class="tableEditButton" @click.stop="toEdit(scope.row.folderId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary" @click="toDetail(scope.row.folderId)">
                  详情
                </el-button>
                <el-button size="small" type="danger" @click="rowDel(scope.row.folderId)">
                  删除
                </el-button>
                <el-button size="small" type="primary"
                           @click="toSon(scope.row.folderId)">
                  其下文件
                </el-button>
              </template>
            </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/basedata/editFilefolder/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/editFilefolder/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/editFilefolder/detail/' + id;
        this.$router.push({path: opath})
      },
      toSon(id) {
        let opath ='/basedata/listEuploadfile?folderId=' + id;
        this.$router.push({path: opath})
      },
      getisSelfStr(row) {
        return row.isSelfStr;
      },
    }
  }
</script>
