      ...
      <el-header style="height: auto">
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          <el-form-item label="发生时间:">
            <el-date-picker v-model="searchForm.happenedDt" type="date"
                            placeholder="选择日期" value-format="yyyy-MM-dd HH:mm" ></el-date-picker>
          </el-form-item>
          <el-form-item label="错误日志:">
            <el-input v-model="searchForm.errorContent" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="发生类名:">
            <el-input v-model="searchForm.commandClass" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="是否已处理:">
            <el-select v-model="searchForm.isHandled" placeholder="请您选择">
              <el-option v-for="isHandledItem in ynOptions"
                         :key="isHandledItem.name"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="happenedDt" label="发生时间" sortable="custom"
                             width="200"></el-table-column>
            <!--<el-table-column prop="errorContent" label="错误日志" sortable="custom" width="200"></el-table-column>-->
            <el-table-column prop="commandClass" label="发生类名" sortable="custom" ></el-table-column>
            <el-table-column prop="isHandled" label="是否已处理" sortable="custom" width="200">
              <template slot-scope="scope">
                <div >
                  <el-switch v-model="scope.row.isHandled == 'Y'" disabled></el-switch>
                </div>
            ...
            <el-table-column label="操作" width="280" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           ...
                           @click.stop="toEdit(scope.row.logId)">
                  编辑
                </el-button>
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.logId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.logId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        let opath = '/safety/detailSystemErrorLog/add/' + 0;
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailSystemErrorLog/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailSystemErrorLog/detail/' + id;
        this.$router.push({path: opath})
      },
      gethappenedDtStr(row) {
        return row.happenedDtStr;
      },
      getisHandledStr(row) {
        return row.isHandledStr;
      },
    }
  }
</script>
