      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="用户:" prop="subjectId">
            <el-input class="input-v2-rao" v-model="searchForm.subjectId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="信息名:" prop="fieldName">
            <el-input class="input-v2-rao" v-model="searchForm.fieldName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="信息值:" prop="fieldValue">
            <el-input class="input-v2-rao" v-model="searchForm.fieldValue" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="subjectId" align="center" label="用户" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.subjectId" class="one-line">{{scope.row.subjectId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="fieldName" align="center" label="信息名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.fieldName" class="one-line">{{scope.row.fieldName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="fieldValue" align="center" label="信息值" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.fieldValue" class="one-line">{{scope.row.fieldValue}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <div class="flex  row">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.subjectIdfieldName)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.subjectIdfieldName)"><i
                  ...
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.subjectIdfieldName)"><i
...
<script>
  import api from "./aasubjectextrafieldhisUrl";
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/safety/detailItemTest/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/safety/ItemTestTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/basedata/detailItemTest/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/basedata/detailItemTest/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
