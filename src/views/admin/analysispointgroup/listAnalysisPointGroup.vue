    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="事件组类型:" prop="pointGroupTypeCode" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.pointGroupTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="时间说明:" prop="pointGroupName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.pointGroupName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="pointGroupTypeCode" label="事件组类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.pointGroupTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.pointGroupTypeCodeStr">{{scope.row.pointGroupTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="pointGroupName" align="center" label="时间说明" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointGroupName" class="one-line">{{scope.row.pointGroupName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.pointGroupId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.pointGroupId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.pointGroupId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/admin/detailAnalysisPointGroup/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/admin/AnalysisPointGroupTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/admin/detailAnalysisPointGroup/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/admin/detailAnalysisPointGroup/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
