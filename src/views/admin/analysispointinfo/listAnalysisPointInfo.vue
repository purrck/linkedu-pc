    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="埋点位置:" prop="pointSite" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.pointSite" maxlength="30"></el-input>
          </el-form-item>
          ...
          <el-form-item label="开始时间:" prop="pointBeginDt" class="w140">
            <el-date-picker class="date-v2-rao" v-model="searchForm.pointBeginDt" type="date"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="pointId" align="center" label="埋点id" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointId" class="one-line">{{scope.row.pointId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="pointSite" align="center" label="埋点位置" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointSite" class="one-line">{{scope.row.pointSite}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="pointBeginDt" label="开始时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="pointEndDt" label="持续时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="pointClicks" align="center" label="点击次数" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointClicks" class="one-line">{{scope.row.pointClicks}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="pointGroupId" align="center" label="事件组id" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointGroupId" class="one-line">{{scope.row.pointGroupId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="pointTrackId" align="center" label="事件组追踪id" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.pointTrackId" class="one-line">{{scope.row.pointTrackId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="userId" align="center" label="用户" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.userId" class="one-line">{{scope.row.userId}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.pointInfoId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.pointInfoId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.pointInfoId)"><i
...
<script>
  import api from "./analysispointinfoUrl";
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/admin/detailAnalysisPointInfo/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/admin/AnalysisPointInfoTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/admin/detailAnalysisPointInfo/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/admin/detailAnalysisPointInfo/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
