      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small">
          ...
          <el-form-item label="使用时间:">
            <el-date-picker v-model="searchForm.recentUsedDt" type="date" placeholder="选择日期"></el-date-picker>
          </el-form-item>
          <el-form-item label="是否有结果:">
            <el-select v-model="searchForm.isRecentUsedHad" placeholder="请您选择">
              <el-option v-for="isRecentUsedHadItem in ynOptions"
                         :key="isRecentUsedHadItem.name"
                         :label="isRecentUsedHadItem.label" :value="isRecentUsedHadItem.name">
              </el-option>
            </el-select>
          </el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="userName" label="用户" sortable="custom" width="200">
              <template slot-scope="scope">
                <div v-if="scope.row.userName">{{scope.row.userName}}</div>
                <div v-else>N/A</div>
              </template>
            </el-table-column>
            <el-table-column prop="usedTimes" label="使用次数" sortable="custom" width="200">
              <template slot-scope="scope">
                <div v-if="scope.row.usedTimes">{{scope.row.usedTimes}}</div>
                <div v-else>N/A</div>
              </template>
            </el-table-column>
            <el-table-column prop="recentUsedDt" label="使用时间" sortable="custom"
                             width="200"></el-table-column>
            <el-table-column prop="isRecentUsedHad" label="是否有结果" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isRecentUsedHad == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
            <el-table-column prop="entityName" label="信息类型" sortable="custom" width="200">
              <template slot-scope="scope">
                <div v-if="scope.row.entityName">{{scope.row.entityName}}</div>
                <div v-else>N/A</div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="350" fixed="right">
              <template slot-scope="scope">
                <el-button size="small" type="primary"
                           @click="toDetail(scope.row.keyword2userId)">
                  详情
                </el-button>
                <el-button size="small" type="danger"
                           @click="rowDel(scope.row.keyword2userId)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/admin/detailSystemSearchKeyword2user/add/' + 0;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/admin/detailSystemSearchKeyword2user/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
