    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="维修时间:" prop="repairDt" class="w140">
                  <el-date-picker class="date-v2-rao" v-model="searchForm.repairDt" type="date"
                                  value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
                </el-form-item>
                <el-form-item label="维修完成时间:" prop="complatedDt" class="w140">
                  <el-date-picker class="date-v2-rao" v-model="searchForm.complatedDt" type="date"
                                  value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
                </el-form-item>
                <el-form-item label="维修人:"  prop="repairUserName" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.repairUserName" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="是否完结:"  prop="isComplated" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isComplated" placeholder="请您选择">
                    <el-option v-for="isComplatedItem in ynOptions"
                               :key="isComplatedItem.name"
                               :label="isComplatedItem.label" :value="isComplatedItem.name">
                    </el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="是否已支付:"  prop="isPayed" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isPayed" placeholder="请您选择">
                    <el-option v-for="isPayedItem in ynOptions"
                               :key="isPayedItem.name"
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="repairDt" label="维修时间" align="center" sortable="custom"></el-table-column>
<!-- 维修时间 colName:repair_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="repairUserName" align="center" label="维修人" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.repairUserName" class="one-line">{{scope.row.repairUserName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 维修人 colName:repair_user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="isComplated" label="是否完结" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isComplated == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
<!-- 是否完结 colName:is_complated sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="payOrderNo" align="center" label="维修订单号" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.payOrderNo" class="one-line">{{scope.row.payOrderNo}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 维修订单号 colName:pay_order_no sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="cost" align="center" label="维修费" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.cost" class="one-line">{{scope.row.cost}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 维修费 colName:cost sqlType:DECIMAL notnull:false -->
                  <el-table-column prop="isPayed" label="是否已支付" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isPayed == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.repairRecordId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.repairRecordId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.repairRecordId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailRepairRecord/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/RepairRecordTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailRepairRecord/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailRepairRecord/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
