      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="身体异常类型:" prop="bodyabnormalTypeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.bodyabnormalTypeCode" placeholder="请您选择">
                ...
                <el-form-item label="已无异常:"  prop="isClosed" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isClosed" placeholder="请您选择">
                    <el-option v-for="isClosedItem in ynOptions"
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="bodyabnormalTypeCode" label="身体异常类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.bodyabnormalTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.bodyabnormalTypeCodeStr">{{scope.row.bodyabnormalTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
...
<!-- 上报时间 colName:bodyabnormal_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="isClosed" label="已无异常" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isClosed == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.bodyabnormalId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.bodyabnormalId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.bodyabnormalId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolStudentBodyabnormal/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolStudentBodyabnormalTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolStudentBodyabnormal/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolStudentBodyabnormal/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
