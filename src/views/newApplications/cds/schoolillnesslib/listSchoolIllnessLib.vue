    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="疾控级别:" prop="illnessControlLevelCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.illnessControlLevelCode" placeholder="请您选择">
                ...
                <el-form-item label="处理方式:" prop="illnessProcessModeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.illnessProcessModeCode" placeholder="请您选择">
                ...
                <el-form-item label="疾病分辨:"  prop="illnessDistinguishVideo" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.illnessDistinguishVideo" maxlength="30"></el-input>
                </el-form-item>
          <el-form-item>
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessTypeCode" label="疾病类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.illnessTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.illnessTypeCodeStr">{{scope.row.illnessTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 疾病类型 colName:illness_type_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessControlLevelCode" label="疾控级别" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.illnessControlLevelCodeStr.split(';')[1]}"
                                  v-if="scope.row.illnessControlLevelCodeStr">{{scope.row.illnessControlLevelCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 疾控级别 colName:illness_control_level_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessProcessModeCode" label="处理方式" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.illnessProcessModeCodeStr.split(';')[1]}"
                                  v-if="scope.row.illnessProcessModeCodeStr">{{scope.row.illnessProcessModeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.illnessLibId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.illnessLibId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.illnessLibId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form : {
          illnessName: null,
          illnessControlLevelCodeValue: null,
          illnessProcessModeCodeValue: null,
          illnessDistinguishVideo: null,
        },
        searchForm : {
          illnessName :null,
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolIllnessLib/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolIllnessLibTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolIllnessLib/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolIllnessLib/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
