...
<template>
  <div class="SchoolEpidemicSituation">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="重大疫情:"  prop="epidemicSituationId" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.epidemicSituationId" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="重大疫情:"  prop="title" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.title" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="疾病名称:"  prop="illnessName" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.illnessName" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="发生时间:" prop="happenedDt" class="w140">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="title" align="center" label="重大疫情" sortable="custom">
...
<!-- 重大疫情 colName:title sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessName" align="center" label="疾病名称" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.illnessName" class="one-line">{{scope.row.illnessName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 疾病名称 colName:illness_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="happenedDt" label="发生时间" align="center" sortable="custom"></el-table-column>
<!-- 发生时间 colName:happened_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="finishedDt" label="截止时间" align="center" sortable="custom"></el-table-column>
<!-- 截止时间 colName:finished_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="studentNum" align="center" label="发病学生数" sortable="custom">
                    <template slot-scope="scope">
...
<!-- 发病学生数 colName:student_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="epidemicSituationStatusCode" label="疫情状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.epidemicSituationStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.epidemicSituationStatusCodeStr">{{scope.row.epidemicSituationStatusCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.epidemicSituationId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.epidemicSituationId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.epidemicSituationId)"><i
                      class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>

                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="重大疫情覆盖学生" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.epidemicSituationId ,`listSchoolEpidemicSituationStudent`)"><i
                          class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>
                    </el-popover>

                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="重大疫情处理小组" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.epidemicSituationId ,`listSchoolEpidemicSituationTeacher`)"><i
                          class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>
                    </el-popover>

                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="重大疫情处理" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.epidemicSituationId ,`listSchoolEpidemicSituationProcess`)"><i
                          class="iconfont icon-icon_tuichu_p ml5 f20 c999999"></i></span>
                    </el-popover>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        this.multipleSelection = val.map(item => {
                                    return item.epidemicSituationId
                                                                                                                                                                                                                                                                                                      });
      },
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolEpidemicSituation/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolEpidemicSituationTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolEpidemicSituation/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolEpidemicSituation/detail/' + id;
        this.$router.push({path: opath})
      },
                  toAnother(id, path) {
        let opath = '/safety/' + path + '?epidemicSituationId=' + id;
        this.$router.push({path: opath})
      },
          }
  }
