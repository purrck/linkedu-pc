...
<template>
  <div class="SchoolStudentIllness">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="疾病:"  prop="illnessName" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.illnessName" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="是否确诊:"  prop="isConfirmed" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isConfirmed" placeholder="请您选择">
                    <el-option v-for="isConfirmedItem in ynOptions"
                               :key="isConfirmedItem.name"
...
<!-- 学生 colName:student_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessName" align="center" label="疾病" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.illnessName" class="one-line">{{scope.row.illnessName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 疾病 colName:illness_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="isConfirmed" label="是否确诊" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isConfirmed == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="happenedDt" label="发病时间" align="center" sortable="custom"></el-table-column>
...
<!-- 发布人 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="illnessProcessModeCode " align="center" label="处理方式" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.illnessProcessModeCode " class="one-line">{{scope.row.illnessProcessModeCode }}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 处理方式 colName:illness_process_mode_code  sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="studentIllnessStatusCode" label="学生生病状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.studentIllnessStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.studentIllnessStatusCodeStr">{{scope.row.studentIllnessStatusCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.studentIllnessId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.studentIllnessId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.studentIllnessId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolStudentIllness/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolStudentIllnessTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolStudentIllness/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolStudentIllness/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
