...
<template>
  <div class="SchoolEpidemicSituationProcess">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="处理:"  prop="esProcessTitle" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.esProcessTitle" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="处理内容:"  prop="content" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.content" maxlength="30"></el-input>
                </el-form-item>
          <el-form-item>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="esProcessTitle" align="center" label="处理" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.esProcessTitle" class="one-line">{{scope.row.esProcessTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 处理 colName:es_process_title sqlType:VARCHAR notnull:false -->
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="processedDt" label="处理时间" align="center" sortable="custom"></el-table-column>
<!-- 处理时间 colName:processed_dt sqlType:TIMESTAMP notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.esProcessId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.esProcessId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.esProcessId)"><i
...
<script>
  import api from "./schoolepidemicsituationprocessUrl";
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolEpidemicSituationProcess/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolEpidemicSituationProcessTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolEpidemicSituationProcess/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolEpidemicSituationProcess/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
