...
<template>
  <div class="SchoolEpidemicSituationTeacher">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="教师职责:" prop="esTeacherDutyCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.esTeacherDutyCode" placeholder="请您选择">
                ...
                <el-form-item label="加入时间:" prop="joinedDt" class="w140">
                  <el-date-picker class="date-v2-rao" v-model="searchForm.joinedDt" type="date"
...
<!-- 教师 colName:teacher_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="esTeacherDutyCode" label="教师职责" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.esTeacherDutyCodeStr.split(';')[1]}"
                                  v-if="scope.row.esTeacherDutyCodeStr">{{scope.row.esTeacherDutyCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 教师职责 colName:es_teacher_duty_code sqlType:VARCHAR notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.esTeacherId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.esTeacherId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.esTeacherId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolEpidemicSituationTeacher/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolEpidemicSituationTeacherTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolEpidemicSituationTeacher/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolEpidemicSituationTeacher/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
