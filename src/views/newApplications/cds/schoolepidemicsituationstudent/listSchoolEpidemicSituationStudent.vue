...
<template>
  <div class="SchoolEpidemicSituationStudent">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                ...
                <el-form-item label="描述:"  prop="description" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.description" maxlength="30"></el-input>
                </el-form-item>
          <el-form-item>
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="studentIllnessId" align="center" label="生病信息id" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.studentIllnessId" class="one-line">{{scope.row.studentIllnessId}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 生病信息id colName:student_illness_id sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.esStudentId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.esStudentId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.esStudentId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/safety/detailSchoolEpidemicSituationStudent/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/safety/SchoolEpidemicSituationStudentTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/safety/detailSchoolEpidemicSituationStudent/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/safety/detailSchoolEpidemicSituationStudent/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
