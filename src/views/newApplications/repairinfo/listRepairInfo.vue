    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="报修信息id:"  prop="repairInfoId" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.repairInfoId" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="报修信息:"  prop="repairInfoName" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.repairInfoName" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="备注:"  prop="repairInfoRemark" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.repairInfoRemark" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="物品类型:" prop="itemTypeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.itemTypeCode" placeholder="请您选择">
                ...
                <el-form-item label="发生区域:"  prop="placeName" class="w140">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="repairInfoName" align="center" label="报修信息" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.repairInfoName" class="one-line">{{scope.row.repairInfoName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
...
<!-- 图片 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="repairInfoRemark" align="center" label="备注" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.repairInfoRemark" class="one-line">{{scope.row.repairInfoRemark}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 备注 colName:repair_info_remark sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="repairBeginDt" label="起始日期" align="center" sortable="custom"></el-table-column>
<!-- 起始日期 colName:repair_begin_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="repairEndDt" label="截至日期" align="center" sortable="custom"></el-table-column>
<!-- 截至日期 colName:repair_end_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="repairStatusCode" label="报修状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.repairStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.repairStatusCodeStr">{{scope.row.repairStatusCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 报修状态 colName:repair_status_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="itemTypeCode" label="物品类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.itemTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.itemTypeCodeStr">{{scope.row.itemTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 物品类型 colName:item_type_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="placeName" align="center" label="发生区域" sortable="custom">
                    <template slot-scope="scope">
...
<!-- 发生区域 colName:place_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="userName" align="center" label="发起人" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 发起人 colName:user_name sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.repairInfoId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.repairInfoId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.repairInfoId)"><i
                      class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>

                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="维修记录" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.repairInfoId ,`listRepairRecord`)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form : {
          repairInfoId :null,
          repairInfoName :null,
          repairInfoRemark :null,
          itemTypeCodeValue: null,
          placeName :null,
        },
        searchForm : {
          repairInfoId :null,
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailRepairInfo/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/RepairInfoTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailRepairInfo/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailRepairInfo/detail/' + id;
        this.$router.push({path: opath})
      },
                  toAnother(id, path) {
        let opath = '/newApplications/' + path + '?repairInfoId=' + id;
        this.$router.push({path: opath})
      },
          }
  }
