    ...
    >
      <el-header style="height: auto">
        ...
        >
          ...
          <el-form-item label="组名:" prop="groupName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.groupName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="groupName" align="center" label="组名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.groupName" class="one-line">{{scope.row.groupName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="isCurrent" label="当前使用" align="center" sortable="custom">
              <template slot-scope="scope">
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover v-if="scope.row.groupSetStatusCode == 'editing'"
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.groupId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.groupId)">
                    <i class="iconfont icon-icon_chakan1 ml5 f20 c999999"></i>
                  </span>
                </el-popover>
                <el-popover v-if="scope.row.SetStatusCode == 'editing'"
                  placement="top-start"
                  popper-class="popover-v2-rao"
                  title="删除"
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.groupId)">
                ...
                >
                  <span
                    slot="reference"
                    @click.stop="toAnother(scope.row.groupId ,`listStudentGroupUser`)"
                  >
...
export default {
  ...
  methods: {
    ...
    toAdd() {
      ...
      let opath = "/newApplications/detailStudentGroup/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/newApplications/StudentGroupTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath = "/newApplications/detailStudentGroup/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/newApplications/detailStudentGroup/detail/" + id;
      this.$router.push({ path: opath });
    },
    toAnother(id, path) {
      let opath = "/newApplications/" + path + "?groupId=" + id;
      this.$router.push({ path: opath });
    }
  }
};
