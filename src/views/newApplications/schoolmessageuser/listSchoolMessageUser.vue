...
<!--                </el-form-item>-->
                ...
                <el-form-item label="状态:" prop="schoolMessageUserStatusCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.schoolMessageUserStatusCode" placeholder="请您选择">
                    <el-option v-for="schoolMessageUserStatusCodeItem in schoolMessageUserStatusCodeOptions"
                               :key="schoolMessageUserStatusCodeItem.name"
                               :label="schoolMessageUserStatusCodeItem.label" :value="schoolMessageUserStatusCodeItem.name">
                    </el-option>
                  </el-select>
                </el-form-item>
...
<!--                <el-form-item label="消息接收时间:"  prop="messageUserDt" class="w140">-->
<!--                  <el-input class="input-v2-rao" v-model="searchForm.messageUserDt" maxlength="30"></el-input>-->
<!--                </el-form-item>-->


          <el-form-item label="发送时间:" prop="messageUserDt" class="w140">
            <el-date-picker class="date-v2-rao" v-model="searchForm.messageUserDt" type="date"
...
<!-- 家长 colName:parent_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="schoolMessageUserStatusCode" label="状态" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.schoolMessageUserStatusCodeStr.split(';')[1]}"
                                  v-if="scope.row.schoolMessageUserStatusCodeStr">{{scope.row.schoolMessageUserStatusCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 状态 colName:school_message_user_status_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="messageReplyContent" align="center" label="回执内容" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.messageReplyContent" class="one-line">{{scope.row.messageReplyContent}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 回执内容 colName:message_reply_content sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="messageUserDt" align="center" label="消息接收时间" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.messageUserDt" class="one-line">{{scope.row.messageUserDt}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 消息接收时间 colName:message_user_dt sqlType:VARCHAR notnull:false -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.messageUserId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.messageUserId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.messageUserId)"><i
...
<script>
  import api from "./schoolmessageuserUrl";
  ...
  export default {
    ...
    mounted() {
      ...
      this.getList();
                                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                                        commonApi.getCodeIntType("schoolMessageUserStatus").then(res => {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailSchoolMessageUser/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/SchoolMessageUserTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailSchoolMessageUser/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailSchoolMessageUser/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
