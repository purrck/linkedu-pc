...
<template>
  <div class>
    <el-container
      ...
      style="min-height: 600px;float: right;width: 90%;"
    ...
    >
      <el-header style="height: auto">
        <el-form
          ...
          class="select-form"
        >
          <el-form-item label="文档索引:" prop="docIndexcId" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.docIndexcId" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          ...
          >
            ...
            <el-table-column prop="docTitle" align="center" label="文件名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.docTitle" class="one-line">{{scope.row.docTitle}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 文件名 -->
            <el-table-column prop="docContent" align="center" label="文章内容" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.docContent" class="one-line">{{scope.row.docContent}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <!-- 文件内容 -->
            <el-table-column prop="publishedDt" align="center" label="发布时间" sortable="custom">
              <template slot-scope="scope">
            ...
            >
              <template slot-scope="scope">
                <span
                  :style="{color:scope.row.schoolDocStatusCodeStr.split(';')[1]}"
                  v-if="scope.row.schoolDocStatusCodeStr"
                >{{scope.row.schoolDocStatusCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <!-- 文件状态 -->
            ...
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toEdit(scope.row.docLibraryId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="toDetail(scope.row.docLibraryId)">
                ...
                <el-popover
                  ...
                  trigger="hover"
                >
                  <span slot="reference" @click.stop="rowDel(scope.row.docLibraryId)">
...
<script>
import api from "./schooldoclibraryUrl";
import commonApi from "@/api/common/common";
import preview from "@/util/preview";
import talk from "../schooldocindex/talk";
export default {
  directives: { preview },
  data() {
    return {
      ...
      msgShow: false,
      timer: null,
      schoolDocStatusCodeOptions: [{ label: "全部", name: "" }],
      form: {
        docIndexcId: null
      },
      searchForm: {
  ...
  mounted() {
    ...
    commonApi.getCodeIntType("schoolDocStatus").then(
      res => {
        let viewList = res.data.data || [];
        for (let schoolDocStatusItem of viewList) {
          this.schoolDocStatusCodeOptions.push({
            label: schoolDocStatusItem.codeDesc,
            name: schoolDocStatusItem.codeId
          });
        }
      },
      res => {
  ...
  methods: {
    ...
    toAdd() {
      let opath = "/newApplications/detailSchoolDocLibrary/add/" + 0;
      this.$router.push({ path: opath });
    },
    toTotal() {
      let opath = "/newApplications/SchoolDocLibraryTotal";
      this.$router.push({ path: opath });
    },
    toEdit(id) {
      let opath = "/newApplications/detailSchoolDocLibrary/edit/" + id;
      this.$router.push({ path: opath });
    },
    toDetail(id) {
      let opath = "/newApplications/detailSchoolDocLibrary/detail/" + id;
      this.$router.push({ path: opath });
    }
    // 自动刷新
  }
