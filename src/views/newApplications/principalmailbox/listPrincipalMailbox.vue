    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="信件标题:"  prop="principalMailTitle" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.principalMailTitle" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="信件类型:" prop="principalMailTypeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.principalMailTypeCode" placeholder="请您选择">
                ...
                <el-form-item label="已读:"  prop="isRead" class="w140">
                ...
                <el-form-item label="已回执:"  prop="isReceipt" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isReceipt" placeholder="请您选择">
                    <el-option v-for="isReceiptItem in ynOptions"
                               :key="isReceiptItem.name"
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="principalMailTitle" align="center" label="信件标题" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.principalMailTitle" class="one-line">{{scope.row.principalMailTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 信件标题 colName:principal_mail_title sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="principalMailTypeCode" label="信件类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.principalMailTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.principalMailTypeCodeStr">{{scope.row.principalMailTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
...
<!-- 已读 colName:is_read sqlType:VARCHAR notnull:false -->
...
<!-- 阅读时间 colName:read_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="isReceipt" label="已回执" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isReceipt == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
<!-- 已回执 colName:is_receipt sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="receiptDt" label="回执时间" align="center" sortable="custom"></el-table-column>
<!-- 回执时间 colName:receipt_dt sqlType:TIMESTAMP notnull:false -->
...
<!-- 家长 colName:parent_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="teacherName " align="center" label="校长姓名" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.teacherName " class="one-line">{{scope.row.teacherName }}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 校长姓名 colName:teacher_name  sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.principalMailId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.principalMailId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.principalMailId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        ynOptions: [{"label": "全部", "name": ""}],
        form : {
          principalMailTitle :null,
          principalMailTypeCodeValue: null,
          ynValue: null,
          ynValue: null,
        },
        searchForm : {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailPrincipalMailbox/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/PrincipalMailboxTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailPrincipalMailbox/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailPrincipalMailbox/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
