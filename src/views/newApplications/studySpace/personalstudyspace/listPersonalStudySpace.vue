    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="个人网络学习空间:"  prop="studySpaceId" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.studySpaceId" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="用户:"  prop="userName" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.userName" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="用户类型:" prop="userTypeCode" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.userTypeCode" placeholder="请您选择">
                ...
                <el-form-item label="空间标题:"  prop="spaceTitle" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.spaceTitle" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="空间口号:"  prop="spaceSlogan" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.spaceSlogan" maxlength="30"></el-input>
                </el-form-item>
          <el-form-item>
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="userName" align="center" label="用户" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.userName" class="one-line">{{scope.row.userName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 用户 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="userTypeCode" label="用户类型" align="center" sortable="custom">
                    <template slot-scope="scope">
                            <span :style="{color:scope.row.userTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.userTypeCodeStr">{{scope.row.userTypeCodeStr.split(';')[0]}}</span>
                      <span v-else>无</span>
                    </template>
                  </el-table-column>
<!-- 用户类型 colName:user_type_code sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="spaceTitle" align="center" label="空间标题" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.spaceTitle" class="one-line">{{scope.row.spaceTitle}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 空间标题 colName:space_title sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="imagePath" align="center" label="主图">
                    <template slot-scope="scope">
...
<!-- 主图 colName:image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="bgImagePath" align="center" label="背景图" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.bgImagePath" class="one-line">{{scope.row.bgImagePath}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 背景图 colName:bg_image_path sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="spaceSlogan" align="center" label="空间口号" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.spaceSlogan" class="one-line">{{scope.row.spaceSlogan}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 空间口号 colName:space_slogan sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="openedDt" label="开通时间" align="center" sortable="custom"></el-table-column>
<!-- 开通时间 colName:opened_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="isOpen" label="是否开放" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isOpen == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
...
<!-- 关注 colName:follow_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="readNum" align="center" label="浏览" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.readNum" class="one-line">{{scope.row.readNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 浏览 colName:read_num sqlType:INTEGER notnull:false -->
                  <el-table-column prop="commendNum" align="center" label="点赞" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.commendNum" class="one-line">{{scope.row.commendNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 点赞 colName:commend_num sqlType:INTEGER notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.studySpaceId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.studySpaceId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.studySpaceId)"><i
                      class="iconfont icon-icon_shanchu_caozuo ml5 f20 c999999"></i></span>
                </el-popover>

                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="空间关注" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.studySpaceId ,`listPersonalStudySpaceFollow`)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailPersonalStudySpace/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/PersonalStudySpaceTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailPersonalStudySpace/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailPersonalStudySpace/detail/' + id;
        this.$router.push({path: opath})
      },
                  toAnother(id, path) {
        let opath = '/newApplications/' + path + '?studySpaceId=' + id;
        this.$router.push({path: opath})
      },
          }
  }
