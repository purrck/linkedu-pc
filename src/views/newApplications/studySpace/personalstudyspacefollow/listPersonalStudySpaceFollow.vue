...
<template>
  <div class="PersonalStudySpaceFollow">
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
                <el-form-item label="空间:"  prop="studySpaceId" class="w140">
                  <el-input class="input-v2-rao" v-model="searchForm.studySpaceId" maxlength="30"></el-input>
                </el-form-item>
                <el-form-item label="关注中:"  prop="isCurrentFollow" class="w140">
                  <el-select class="select-v2-rao" v-model="searchForm.isCurrentFollow" placeholder="请您选择">
                    <el-option v-for="isCurrentFollowItem in ynOptions"
                               :key="isCurrentFollowItem.name"
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="baseUserName" align="center" label="被关注用户" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.baseUserName" class="one-line">{{scope.row.baseUserName}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
<!-- 被关注用户 colName:base_user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="userName" align="center" label="关注人" sortable="custom">
                    <template slot-scope="scope">
...
<!-- 关注人 colName:user_name sqlType:VARCHAR notnull:false -->
                  <el-table-column prop="followedBeiginDt" label="关注时间" align="center" sortable="custom"></el-table-column>
<!-- 关注时间 colName:followed_beigin_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="followedColseDt" label="解关时间" align="center" sortable="custom"></el-table-column>
<!-- 解关时间 colName:followed_colse_dt sqlType:TIMESTAMP notnull:false -->
                  <el-table-column prop="isCurrentFollow" label="关注中" align="center" sortable="custom">
                    <template slot-scope="scope">
                      <el-switch v-model="scope.row.isCurrentFollow == 'Y'" disabled></el-switch>
                    </template>
                  </el-table-column>
<!-- 关注中 colName:is_current_follow sqlType:VARCHAR notnull:false -->
            <el-table-column label="操作" fixed="right">
              <template slot-scope="scope">

                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.spaceFollowId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.spaceFollowId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.spaceFollowId)"><i
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/newApplications/detailPersonalStudySpaceFollow/add/' + 0;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/newApplications/PersonalStudySpaceFollowTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/newApplications/detailPersonalStudySpaceFollow/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/newApplications/detailPersonalStudySpaceFollow/detail/' + id;
        this.$router.push({path: opath})
      },
                }
  }
