    ...
    <el-container v-loading="isLoaded" element-loading-custom-class="home-loading"
      ...
      <el-header style="height: auto">
        ...
        <el-form ref="searchForm" :model="searchForm" :inline="true" size="small" class="select-form">
          <el-form-item label="姓名:" prop="visitorName" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.visitorName" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="证件类型:" prop="cardTypeCode" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.cardTypeCode" placeholder="请您选择">
          ...
          <el-form-item label="证件编号:" prop="cardNum" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.cardNum" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="电话:" prop="visitorPhonenum" class="w140">
            <el-input class="input-v2-rao" v-model="searchForm.visitorPhonenum" maxlength="30"></el-input>
          </el-form-item>
          <el-form-item label="来访时间:" prop="visitedDt" class="w140">
            <el-date-picker class="date-v2-rao" v-model="searchForm.visitedDt" type="date"
                            value-format="yyyy-MM-dd HH:mm" placeholder="选择日期"></el-date-picker>
          </el-form-item>
          <el-form-item label="是否家长:" prop="isParent" class="w140">
            <el-select class="select-v2-rao" v-model="searchForm.isParent" placeholder="请您选择">
              <el-option v-for="isParentItem in ynOptions"
                         :key="isParentItem.name"
      ...
      <el-main class="pt-15">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column prop="imagePath" align="center" label="头像">
              <template slot-scope="scope" style="min-width: 100px">
                <div v-if="scope.row.imagePath" >
                  <img :src="scope.row.imagePath" alt="" style="width: 100%" >
                </div>
                <div v-if="!scope.row.imagePath">无</div>
              </template>
            </el-table-column>
            <el-table-column prop="visitorName" align="center" label="姓名" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.visitorName" class="one-line">{{scope.row.visitorName}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="cardTypeCode" label="证件类型" align="center" sortable="custom">
              <template slot-scope="scope">
                            <span :style="{color:scope.row.cardTypeCodeStr.split(';')[1]}"
                                  v-if="scope.row.cardTypeCodeStr">{{scope.row.cardTypeCodeStr.split(';')[0]}}</span>
                <span v-else>无</span>
              </template>
            </el-table-column>
            <el-table-column prop="cardNum" align="center" label="证件编号" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.cardNum" class="one-line">{{scope.row.cardNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="visitorPhonenum" align="center" label="电话" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.visitorPhonenum" class="one-line">{{scope.row.visitorPhonenum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="visitedDt" label="来访时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="leftDt" label="离开时间" align="center" sortable="custom"></el-table-column>
            <el-table-column prop="visitCardNum" align="center" label="访问卡号" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.visitCardNum" class="one-line">{{scope.row.visitCardNum}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="remark" align="center" label="来访事由" sortable="custom">
              <template slot-scope="scope">
                <div v-if="scope.row.remark" class="one-line">{{scope.row.remark}}</div>
                <div v-else>无</div>
              </template>
            </el-table-column>
            <el-table-column prop="isParent" label="是否家长" align="center" sortable="custom">
              <template slot-scope="scope">
                <el-switch v-model="scope.row.isParent == 'Y'" disabled></el-switch>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作"  fixed="right">
              <template slot-scope="scope">
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.visitRecordId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.visitRecordId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.visitRecordId)"><i
  ...
  export default {
    ...
    data() {
      return {
        ...
        form: {
          visitorName: null,
          cardTypeCodeValue: null,
          cardNum: null,
          visitorPhonenum: null,
          visitedDt: null,
          ynValue: null,
        },
        searchForm: {
    ...
    methods: {
      ...
      toAdd() {
        ...
        let opath = '/dormitory/detailSchoolVisitRecord/add/' + 0;
        this.$router.push({path: opath})
      },
      toTotal() {
        let opath = '/dormitory/SchoolVisitRecordTotal';
        this.$router.push({path: opath})
      },
      toEdit(id) {
        let opath = '/dormitory/detailSchoolVisitRecord/edit/' + id;
        this.$router.push({path: opath})
      },
      toDetail(id) {
        let opath = '/dormitory/detailSchoolVisitRecord/detail/' + id;
        this.$router.push({path: opath})
      },
    }
  }
