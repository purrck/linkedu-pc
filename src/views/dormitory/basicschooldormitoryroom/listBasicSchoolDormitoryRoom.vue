...
<template>
  <div class="BasicSchoolDormitoryRoom">
        ...
        <div class="table">
          <el-table :data="data.records" tooltip-effect="dark" style="width: 100%" border
            ...
            <el-table-column type="index" :index="indexMethod" label="序号" width="65" align="center"></el-table-column>
                  <el-table-column prop="dormitoryRoomNum" align="center" label="房间号" sortable="custom">
                  ...
                  <el-table-column prop="studentNum" align="center" label="学生人数" sortable="custom">
                    <template slot-scope="scope">
                      <div v-if="scope.row.studentNum" class="one-line">{{scope.row.studentNum}}</div>
                      <div v-else>无</div>
                    </template>
                  </el-table-column>
                  <el-table-column prop="maxUser" align="center" label="可住人数" sortable="custom">
                    <template slot-scope="scope">
            ...
            <el-table-column label="操作" >
              <template slot-scope="scope">
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="编辑" trigger="hover">
                    <span slot="reference"
                          @click.stop="toEdit(scope.row.dormitoryRoomId)"><i
                  ...
                  <el-popover placement="top-start" popper-class="popover-v2-rao" title="查看" trigger="hover">
                    <span slot="reference"
                          @click.stop="toDetail(scope.row.dormitoryRoomId)"><i
                ...
                <el-popover placement="top-start" popper-class="popover-v2-rao" title="删除" trigger="hover">
                  <span slot="reference"
                        @click.stop="rowDel(scope.row.dormitoryRoomId)"><i
                    ...
                    <el-popover placement="top-start" popper-class="popover-v2-rao" title="学生住宿" trigger="hover">
                      <span slot="reference" @click.stop="toAnother(scope.row.dormitoryRoomId ,`listBasicSchoolDormitoryStudent`,scope.row.dormitoryRoomNum)"><i
...
<script>
  import api from "./basicschooldormitoryroomUrl";
  ...
  export default {
    ...
    methods: {
      ...
      dialogDeleteCancelHandler() {
        ...
        let opath = '/dormitory/detailBasicSchoolDormitoryRoom/add/' + this.$route.query.dormitoryId+'?dormitoryNum='+this.$route.query.dormitoryNum+'&dormitoryName='+this.$route.query.dormitoryName;
        this.$router.push({path: opath})
      },
            toTotal (){
        let opath = '/dormitory/BasicSchoolDormitoryRoomTotal';
        this.$router.push({path: opath})
      },
            toEdit(id) {
        let opath = '/dormitory/detailBasicSchoolDormitoryRoom/edit/' + id;
        this.$router.push({path: opath})
      },
                  toDetail(id) {
        let opath = '/dormitory/detailBasicSchoolDormitoryRoom/detail/' + id;
        this.$router.push({path: opath})
      },
                  toAnother(id, path,num) {
        let opath = '/dormitory/' + path + '?dormitoryRoomId=' + id+'&dormitoryRoomNum='+num+'&dormitoryId='+this.$route.query.dormitoryId+'&dormitoryNum='+this.$route.query.dormitoryNum+'&dormitoryName='+this.$route.query.dormitoryName;
        this.$router.push({path: opath})
      },
          }
  }
