<template>
  <div class="sendObject-wrap">
    <header>
      <ul>
        <li :class="{active: item.isActive}" v-for="(item, index) of titleData" class="iconfont" :key="index">
    ...
    <section class="main">
      ...
      <div class="main__menu">
        <ul class="main__menu__box">
          <li class="main__menu__box__item" :class="{active: item.isActive}" v-for="(item, index) of listTreeData"
              @click="handleClick(item, index)" :key="index">{{item.subTitle}}
          </li>
        </ul>
      </div>

      <div class="main__content" style="min-width: 400px">
        <div v-for="(item, index) of listTreeData" v-show="item.isActive&&!item.single" :key="index">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      listTreeData: {
        get () {
          return this.header[this.index].children;
        }
      },
    },
    data() {
    ...
    methods: {
      ...
      handleClick(item, index) {
        ...
        !item.children.length && this.getItemChildren(item)
      },
      getItemChildren (item) {
        let url = '';
        switch (item.subTitleType) {
          case 'segment':
            url =  `/sm/base/schoolSegmentAd/segmentGrade?type=2&id=${item.id}`
            break;
          case 'grade':
            url =  `/sm/base/classInfoAd/list?gradeId=${item.id}`
            break;
          case 'class':
            url =  `/sm/user/schoolPerson/option?roleId=F&classIds=${item.id}`
            break;
          case 'students':
            url =  `/sm/user/schoolPerson/listStudentParent?ids=${item.id}&isGetParent=true`
            break;
          case 'province':
            url =  `/userapp/base/getArea?areaType=3&id=${item.id}`
            break;
          case 'city':
            url =  `/userapp/base/getArea?areaType=4&id=${item.id}`
            break;
          case 'area':
            url = `/userapp/base/getArea?areaType=4&id=${item.id}`
            break;
      ...
      next() {
        ...
        //       })
        //     })
        //     this.treeData[this.index].children = [];
        //     break;
        // }
        // this.initData()
