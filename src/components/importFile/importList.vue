    ...
    <div style="width:70%;margin:0 auto 0;">
      <el-main>
        <div v-if="state == 'all'">
          ...
          <el-table
            :data="allList"
            class="table-rao checkbox-rao mt-15"
            ref="schooltable"
            tooltip-effect="dark"
            style="width: 100%"
            border
        ...
        <div v-if="state == 'fail'">
          <div class="importHead">
            <img
              src="~@/assets/img/icon_chengong.png"
              style="vertical-align: -9px;padding-right:10px;"
              width="30"
            />导入数据失败
            <span style="color:#CB3946">{{failCount}}</span> 条
          </div>
          <el-table
            :data="failData"
            class="table-rao checkbox-rao"
            ref="schooltable"
            tooltip-effect="dark"
            style="width: 100%"
            border
