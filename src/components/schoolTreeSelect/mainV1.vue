        ...
        methods : {
            ...
            classClick(index){
                if(this.classList.length){
                    ...
                    this.subjectList = this.classList[index].children;
                }else{
                    this.subjectList = "";
                }
            },
            ...
            getSchoolTree(){
                commonApi.getSchoolTree().then((res) => {
                    if(res.data.resultCode === '000000' ){
                        this.watcher = this.msg.schoolData = res.data.data;
                        this.segmentList = res.data.data.children;

                        let _count=1;
                        let _arr=[res.data.data.children];
                        let _a = (children) => {
                            if(children.length > 0){
                                _arr.push(children[_count].children)
                                _a(children[_count].children);
                                _count++;
                            }
                        }
                        ...
                        for(let i=0; i<this.segmentList.length; i++){
                            if(this.segmentList[i].id == this.selected.segmentId){
                                ...
                                this.formData.segment = this.selected.segmentId;
                            }
                            for(let j=0; j<this.segmentList[i].children.length; j++){
                                if(this.segmentList[i].children[j].id == this.selected.classId){
                                    this.watcher = this.msg.class = this.segmentList[i].children[j];
                                    this.classList = this.segmentList[i].children;
                                    this.formData.class = this.selected.classId;
                                }
                                for(let s=0; s<this.segmentList[i].children[j].children.length; s++){
                                    if(this.segmentList[i].children[j].children[s].id == this.selected.subjectId){
                                        ...
                                        this.formData.subject = this.selected.subjectId;
                                    }
                                }
                            }
                        }
